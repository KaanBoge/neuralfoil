# Historical Studio description

The text below is the original README from commit `08fcd189d5636cd9a499141981a09e254fde1574`, retained verbatim for historical context. Its labels and validation claims describe that earlier browser/study release, not the newer drag-correction code archive. They are not a new review or an endorsement of universal accuracy. See [current status](STATUS.md) for the distinction.

Historical links below retain their original repository-root context. Use [the current study directory](../study/) for working navigation. The cited `study/data/research-answer.md` is one of three legacy paper records excluded from the current public tree, along with `study/NeuralFoil-Study-Complete-Record.pdf` and `study/docs/paper-outline-FINAL.md`. They are not current downloads. Original working copies and already-public Git history remain preserved; the quoted README body is unchanged.
