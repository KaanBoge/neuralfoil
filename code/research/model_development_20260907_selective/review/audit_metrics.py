...
PROJECT=ROOT.parent
A=ROOT/'assessment'
CANDIDATES=['project_mean8','project_xlarge']
REFS=['unpenalized_transfer','half_strength']
LABELS=CANDIDATES+['proper_core','proper_half']+REFS
...
def source_join(d):
    ...
    for split in d.split.unique():
        ...
        for key in CANDIDATES+['proper_core','proper_half','measured_CD']+BASES:
...
def main():
    ...
    assert frozen['core_count']==16 and frozen['external_outcomes_opened'] is False
    ...
    for seed in SEEDS:
        ...
        draws=np.random.default_rng(2026090727).multinomial(93,np.full(93,1/93),size=20000)
        ...
        for label in LABELS:
            ...
            for ref in REFS:
                ...
                compare(bt.loc[label,seed,ref],{'rows':8371,'groups':93,'remaining_MAE_reduction_percent':gain,'conditional_95pct_lower':lo,'conditional_95pct_upper':hi,'bootstrap_fraction_benefit_positive':float((benefit>0).mean()),'draws':20000,'seed':2026090727},(label,seed,ref));checks['bootstraps']+=1
    verdicts=[]
