...
def authorize(args):
    freeze=json.loads(checked(HERE/'IMPLEMENTATION_FREEZE_v2.json',args.implementation_sha256))
...
class PhaseFiles:
    """Parse exact predecessor-authenticated bytes; log every actual repeated read."""
    def __init__(self,phase,approval):
        self.events=[];self.root=None;self.hashes={}
        prior={'score':'calibrate','assess':'score'}.get(phase)
        if prior:
            self.root=HERE/prior
            record=json.loads(checked(self.root/'COMPLETE.json',approval['predecessor_sha256']))
            self.hashes=record['outputs']
    def raw(self,name,parser):
        if name not in self.hashes or Path(name).name!=name:raise ValueError('unregistered phase file')
        path=self.root/name;raw=checked(path,self.hashes[name])
        self.events.append({'path':str(path),'sha256':self.hashes[name],'parser':parser,'bytes':len(raw)})
        return raw
...
def score(inputs,out,codec,c):
    for context in inputs.native:
        ...
        for label in NEW_LABELS:
            model=inputs.phase_files.json(f'calibrator_{label}_{scalar_context}.json')
...
def assess(inputs,out,codec,c):
    ...
    panels={k:np.array(v,int) for k,v in inputs.metadata('scoring/panels.json').items()}
    if len(frame)!=29856 or len(panels)!=31:raise ValueError('frame inventory')
    frame=overlay(frame,{split:inputs.phase_files.csv(f'predictions_{split}.csv') for split in frame.split.unique()})
...
def main():
    ...
    try:
        inputs=Inputs(ARCHIVE,args.phase);inputs.phase_files=PhaseFiles(args.phase,approval)
        c=module('kl_production',checked(CONFIDENCE,CONFIDENCE_SHA),CONFIDENCE)
        ...
        authorize(args);predecessor(args.phase,approval,args.implementation_sha256)
        checked(ARCHIVE,ZIP_SHA)
        record.update(finished_UTC=now(),seconds=time.monotonic()-start,archive_materializations=inputs.events,
            phase_file_reads=inputs.phase_files.events,end_provenance_reauthenticated=True,
        ...
        record.update(exception=repr(exc),traceback=traceback.format_exc(),finished_UTC=now(),seconds=time.monotonic()-start,
            archive_materializations=inputs.events if inputs else [],
            phase_file_reads=inputs.phase_files.events if inputs and hasattr(inputs,'phase_files') else [])
