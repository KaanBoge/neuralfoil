...
class Successor(unittest.TestCase):
    def test_builder_inventory_uses_logged_reader(self):
        import ast
        source=Path(__file__).with_name('build_replay.py').read_text();tree=ast.parse(source)
        function=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='build')
        node=next(n for n in function.body if isinstance(n,ast.Assign) and any(isinstance(t,ast.Name) and t.id=='info' for t in n.targets))
        expected=ast.parse("info=shared.Reader(parent,ledger,'parent').json('inventory.json')").body[0]
        self.assertEqual(ast.dump(node,include_attributes=False),ast.dump(expected,include_attributes=False))
