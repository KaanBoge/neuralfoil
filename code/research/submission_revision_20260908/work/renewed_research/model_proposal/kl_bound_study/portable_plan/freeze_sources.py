...
def main():
    ...
    prior_raw=(HERE/'v2snapshot/REGISTRY_v2.json').read_bytes()
    if integrity.sha(prior_raw)!='7d586bb5c3cfa3021bacc9c70c2251be61f3e22ca4066885cbe601dbf0007188':raise ValueError('v2 snapshot registry')
    prior=json.loads(prior_raw);delta={}
    for n,h in prior['sources'].items():
        old=(HERE/'v2snapshot'/n).read_bytes();new=(HERE/n).read_bytes()
        if integrity.sha(old)!=h:raise ValueError('v2 snapshot source')
        delta[n]={'prior_sha256':h,'successor_sha256':integrity.sha(new),'diff':''.join(difflib.unified_diff(old.decode().splitlines(True),new.decode().splitlines(True),fromfile='v2snapshot/'+n,tofile=n))}
    with (HERE/'SOURCE_DIFF_v3.json').open('x') as f:json.dump(delta,f,indent=2);f.write('\n')
    names=['PLAN.md','README.md','COMPLETED_REVIEW_NOTE.md','integrity.py','export_support.py','shared.py','build_replay.py','replay.py','test_portable.py','test_source_adapter.py','freeze_sources.py','SOURCE_HANDOFF.md','test_successor.py','SUCCESSOR_FAILURE_1.md','SUCCESSOR_HANDOFF.md','SOURCE_DIFF_v2.json','SOURCE_DIFF_v3.json','V3_HANDOFF.md']
    z={'status':'SOURCE_ONLY_AWAITING_REVIEW','sources':{n:integrity.sha((HERE/n).read_bytes()) for n in names},'inputs':inputs,'evidence':evidence,
        ...
        'preserved_registry_sha256':integrity.sha(prior_raw),'synthetic_tests':26}
    raw=json.dumps(z,indent=2).encode()+b'\n'
    with (HERE/'REGISTRY_v3.json').open('xb') as f:f.write(raw)
