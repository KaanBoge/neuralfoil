...
def engines(parent,addon):
    ...
    codec=module('codec',parent['code/codec.py']);n=module('parent_policy',parent['code/policy.py'])
    kl=module('exact_kl',addon['code/exact_kl.py'])
    c=module('kl_adapter',addon['code/adapter.py'],{'kl':kl,'kl_raw':addon['code/exact_kl.py'],'n':n})
    frame=module('frame_codec',parent['code/frame_codec.py'])
    overlay=module('typed_overlay',addon['code/overlay.py'])
    metrics=module('metrics',parent['code/metrics.py'])
...
def compare_tables(name,table,raw):
    ...
    for col in actual:
        ...
        if pd.api.types.is_numeric_dtype(x) and x.dtype!=bool:
            ...
            for i in np.flatnonzero((x.to_numpy()!=y.to_numpy())&~x.isna().to_numpy()):
                ...
                if pd.api.types.is_integer_dtype(x):raise ValueError('integer count mismatch')
