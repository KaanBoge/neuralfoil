...
def main():
    ...
    for name,h in EXTERNAL.items():
        ...
        external[str(p)]=h
    old_raw=(HERE/'v1snapshot/IMPLEMENTATION_FREEZE.json').read_bytes()
    if sha(old_raw)!='fe6c11545cac40f496d5caed022e18a08e51b0b61392f6a286fd225b85a023d9':raise ValueError('snapshot freeze')
    previous=json.loads(old_raw);delta={}
    for name,h in previous['source_sha256'].items():
        old=(HERE/'v1snapshot'/name).read_bytes();new=(HERE/name).read_bytes()
        if sha(old)!=h:raise ValueError('snapshot source')
        delta[name]={'previous_sha256':h,'successor_sha256':sha(new),'unchanged':old==new,
            'unified_diff':''.join(difflib.unified_diff(old.decode().splitlines(True),new.decode().splitlines(True),fromfile='v1snapshot/'+name,tofile=name))}
    delta_raw=json.dumps(delta,indent=2).encode()+b'\n'
    with (HERE/'SOURCE_DIFF_v2.json').open('xb') as f:f.write(delta_raw)
    names=['inputs.py','run_experiment.py','test_study.py','freeze_sources.py','IMPLEMENTATION_PLAN.md','PROTOCOL_ADDENDUM.md','SYNTHETIC_RESULTS.md','SUCCESSOR_REVIEW.md','SOURCE_DIFF_v2.json']
    record={'status':'SOURCE_ONLY_AWAITING_REVIEW_AND_PHASE_APPROVALS',
        ...
        'source_only_synthetic_tests':31,'preserved_v1_freeze_sha256':sha(old_raw)}
    raw=json.dumps(record,indent=2).encode()+b'\n'
    with (HERE/'IMPLEMENTATION_FREEZE_v2.json').open('xb') as f:f.write(raw)
    print(sha(raw))
