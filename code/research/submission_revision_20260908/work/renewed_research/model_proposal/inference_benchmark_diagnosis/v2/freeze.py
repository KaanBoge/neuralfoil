...
def main():
    ...
    oldpin='237b64cb484d5504f6003a9939a1a31167b0e69d8b25ad99a0f1f07be8c1df5a'
    old=d.parse(d.read(here.parent/'v1/REGISTRY.json',oldpin,ledger),'preserved V1 registry',ledger)
    for name,pin in old['sources'].items():d.read(here.parent/'v1'/name,pin,ledger)
    d.read(here/'diagnostic.py',old['sources']['diagnostic.py'],ledger)
    ...
    out=here/'synthetic_attempt_2';out.mkdir(exist_ok=False)
    ...
    sources={name:d.sha((here/name).read_bytes()) for name in ['PROTOCOL.md','diagnostic.py','run.py','test_diagnostic.py','freeze.py','V2_ADDENDUM.md']}
    reg={'schema':'SG_DIAGNOSTIC_SOURCE_V2','v1_registry_sha256':oldpin,'unchanged_diagnostic_math_sha256':old['sources']['diagnostic.py'],'sources':sources,'inputs':inputs,'roles':roles,'members':['SG_exposed_'+k for k in d.KEYS],
      'runtime':run.runtime(),'output_allowlist':sorted(d.OUT_NAMES),'output_cap_bytes':d.CAP,'seconds':120,'workers':1,'rows':242,
      'synthetic_evidence_sha256':d.sha((out/'DIAGNOSTICS.json').read_bytes()),'preserved_failure_sha256':d.sha((here/'synthetic_attempt_1/DIAGNOSTICS.json').read_bytes()),'prefreeze_failure_sources':{name:d.sha((here/'prefreeze_failure_1'/name).read_bytes()) for name in ['test_diagnostic.py','freeze.py']},'source_metadata_reads':ledger,'actual_execution_authorized':False}
