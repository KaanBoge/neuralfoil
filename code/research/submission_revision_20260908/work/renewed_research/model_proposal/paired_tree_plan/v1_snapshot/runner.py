...
def protected_attempt(out,metadata,work,seconds=900):
    ...
    s.exclusive(out/'ATTEMPT.json',dict(metadata,start_utc=started,python=platform.python_version(),pid=os.getpid()))
...
def execute(args):
    regpath=s.HERE/'REGISTRY.json';approvalpath=s.safe_path(args.approval)
    early=[]
    ...
    def work(out,ledger,outputs):
        ...
        # Exact whole-manifest identity is inherited; require named model pin occurs too.
        if s.MODEL_SHA not in json.dumps(manifest):raise ValueError('manifest missing model pin')
