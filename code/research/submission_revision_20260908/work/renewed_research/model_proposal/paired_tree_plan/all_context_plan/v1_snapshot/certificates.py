...
def phase_inputs(args,ledger):
    reg=a.json_read(a.HERE/'REGISTRY.json',args.registry_sha256,ledger)
...
def child(args):
    ...
    def work(out,ledger,outputs,deadline):
        ...
        with a.sole_modules(ledger) as (ms,oldreg):
            ...
            if args.phase=='certificates_produce':
                ...
                result={'certificate_sha256':pin,'counts':checked['counts'],'summary':checked['summary'],'memory':memory,'status':'PASS_CONTEXT_REPLAY'}
    ...
    return a.attempt(args.output,{'phase':args.phase,'context':args.context,'registry_sha256':args.registry_sha256,'approval_sha256':args.approval_sha256},work)
...
def _communicate(p,remaining):
    stdout,stderr=p.communicate(timeout=remaining);return p.returncode,stdout.decode(errors='replace'),stderr.decode(errors='replace')

def main_phase(args):
    def work(out,ledger,outputs,deadline):
        reg,ap,table=phase_inputs(args,ledger)
        disk=a.disk_preflight(out);entries={}
        if args.phase=='certificates_replay':
            prior=a.json_read(a.HERE/'certificates_produce/COMPLETE.json',ap['predecessor_sha256'],ledger)
            if prior['status']!='COMPLETE' or set(prior['summary']['contexts'])!=set(a.CONTEXTS):raise ValueError('all producers barrier')
        entries['final']=a.inherited_final(ledger,args.phase=='certificates_replay')
        for row in table[:-1]:
            context=row['context'];left=deadline-time.monotonic()
            ...
            code,stdout,stderr=run_child(command,left)
            outputs[f'process_{context}.json']=a.save(out/f'process_{context}.json',{'returncode':code,'stdout':stdout,'stderr':stderr})
            if code!=0:raise RuntimeError('context failed; remaining NOT_RUN: '+context)
        ...
        a.authenticate(reg,args.registry_sha256,args.approval,args.approval_sha256,args.phase,ledger)
        return {'contexts':entries,'disk_preflight':disk,'logical_output_bytes':a.logical_usage(out),'fresh_contexts':15,'inherited_contexts':1,'features_targets_loaded':0,'fits':0}
