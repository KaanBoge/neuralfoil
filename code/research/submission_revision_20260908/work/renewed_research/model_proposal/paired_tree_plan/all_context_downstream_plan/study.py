...
def complete(phase,pin,regsha,ledger):
    ...
    if (r.get('status')!='COMPLETE' or r.get('registry_sha256')!=regsha or
        r.get('phase')!=phase or r.get('inherited_certificate_registry_sha256')!=a.LEGACY_SHA or
        (root/'FAILURE.json').exists()):raise ValueError('phase completion identity')
    for name,h in r['outputs'].items():
        if Path(name).name!=name or name not in a.names(phase):raise ValueError('receipt output name')
        a.read(root/name,h,ledger,'completed output authentication')
    previous=r.get('summary',{}).get('predecessor_sha256')
...
def calibrate(parent,addon,engines,prior,out,ledger,outputs):
    ...
    for ctx in a.CONTEXTS:
        ...
        for label,result in zip(si.LABELS,results):
            ...
            name=f'calibrator_{label}_{ctx}.json';outputs[name]=a.scalar(out/name,result,codec)
...
def score(parent,addon,engines,prior,out,ledger,outputs):
    ...
    for ctx in info['native_contexts']:
        ...
        outputs[name]=a.npz(out/name,{**z,**{k:v for k,v in table.items() if k not in z}})
        if ctx!='final':
            if ctx not in si.EXT:table['nf2_row_id']=parent.json('roles/'+ctx+'.json')['test_nf2_row_ids']
            name=f'predictions_{ctx}.csv';outputs[name]=a.csv(out/name,pd.DataFrame(table))
...
def assess(parent,addon,engines,prior,out,ledger,outputs):
    ...
    for name,t in tables.items():
        path=out/(name+'.csv');outputs[path.name]=a.csv(path,t)
