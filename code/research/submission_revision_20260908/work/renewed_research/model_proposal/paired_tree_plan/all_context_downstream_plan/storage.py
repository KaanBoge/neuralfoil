...
class Binary:
    ...
    def read(self, *args):
        # NumPy recognizes file-like objects by this attribute; output stays write-only.
        raise OSError('guarded output is write-only')
...
def failure_payload(meta, exc, outputs, ledger):
    ...
    first = exc.args[0] if exc.args and type(exc.args[0]) is str else ''
    return dict(status='FAILED_STOP_NO_RETRY', phase=str(meta.get('phase', ''))[:32],
                error_type=name, detailed_error_not_serialized=True,
                message_prefix=first[:256], message_characters=len(first),
                message_truncated=len(first) > 256,
