...
class Tests(unittest.TestCase):
    ...
    def test_preallocation_gate(self):
        ...
        self.assertGreater(r['estimated_owned_peak_bytes'],128*2**20)
        w.enforce_memory(r)
        with self.assertRaises(w.MemoryBudgetError):w.enforce_memory(dict(r,estimated_owned_peak_bytes=w.LIMIT+1))
    def test_memory_refusal_before_parser(self):
        raw=b'"empty":'*6000+b'"threshold":'*47600+b'"leaves":'*90000
        buf=io.BytesIO();np.savez(buf,**fixtures.small_arrays())
        with tempfile.TemporaryDirectory() as td:
            with patch.object(w,'parse_json',side_effect=AssertionError('must not parse')),patch.object(w,'memory_plan',return_value={'estimated_owned_peak_bytes':w.LIMIT+1}):
                with self.assertRaises(w.MemoryBudgetError):w.replay_buffers(raw,buf.getvalue(),b'',b'','',[],{},Path(td).resolve(),'0'*64,time.monotonic()+10)
    def test_amended_cap_only(self):
        self.assertEqual(w.LIMIT,268435456)
        with self.assertRaises(MemoryError):s.Budget().check(134217729)
