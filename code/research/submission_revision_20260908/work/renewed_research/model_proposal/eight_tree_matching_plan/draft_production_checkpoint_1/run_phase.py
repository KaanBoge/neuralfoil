...
def approval(a, phase, regsha, entrysha):
    ...
    extra = {'source_review_sha256'} | ({'cold_gate_sha256'} if phase == 'producer' else set())
...
def execute(args):
    ...
    try:
        ...
        a = s.parse(s.read(apath, args.approval_sha256, ledger, 2**20), ledger, 'approval')
        ...
        review = s.parse(s.read(rpath, a['source_review_sha256'], ledger, 2**20), ledger, 'review')
        ...
        if args.phase == 'producer':
            ...
            gate = s.parse(s.read(gatepath, a['cold_gate_sha256'], ledger, 2**20), ledger, 'fixed gate')
            if gate.get('status') != 'PASS_FIXED_315000_GATE' or gate.get('registry_sha256') != args.registry_sha256 or gate.get('pair_classifications') != 315000:
        ...
        if args.phase == 'capsule':
            ...
            for key, entry in reg['inputs'].items():
                if key != 'old_certificate':
                    ...
                    refs[key] = s.parse(data, ledger, key)
        ...
        store.json('ACCESS.json', ledger)
                    ...
                    outputs=dict(store.outputs), summary=result)
        ...
        if store is not None:
            store.close_partial()
            store.json('FAILURE.json', dict(metadata, status='FAILED_NO_RETRY', finish_utc=utc(),
                       elapsed_seconds=time.monotonic()-start, error=repr(exc)[:4096],
                       accepted_outputs=dict(store.outputs), last_access=ledger[-30:]), emergency=True)
        raise
    finally:
        signal.setitimer(signal.ITIMER_REAL, 0)
