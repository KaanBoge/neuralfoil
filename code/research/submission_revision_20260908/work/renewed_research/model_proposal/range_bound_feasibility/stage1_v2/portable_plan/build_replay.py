...
def authorize(args):
    frozen=json.loads(checked(HERE/'REGISTRY_v3.json',args.registry_sha256))
...
def build(args):
    frozen=authorize(args)
    with export_support.ReaderTrace(frozen['inputs']) as trace:
        args.data_reads=trace.events
        return build_frozen(args,frozen)

def build_frozen(args,frozen):
    payload={};witness=[];access=[]
    ...
    put('SOURCE_WITNESS.json',jsonbytes({'functions':witness,'original_preassessment_prefix_AST_sha256':prefix_witness,'inputs':frozen['inputs'],'accessed':access,
        'actual_data_reads':args.data_reads,'tracing_scope':'All numpy.load/pandas.read_csv calls in authenticated export scope; NPZ members recorded only when materialized; copied bytes are separately listed as accessed.'}))
...
if __name__=='__main__':
    ...
    export_support.run_attempt(a,guarded_build)
