...
def roots():
    paired=HERE.parents[1]/'paired_tree_plan/all_context_downstream_plan'
    certificates=HERE.parent/'all_context_proposal/implementation'
    return ([old.HERE/p for p in old.PHASE_DIRS]+[paired/p for p in PHASES]+[paired/'execution_evidence']+
            [certificates/p for p in ['certificates_produce','certificates_replay']]+
            [HERE/p for p in PHASES]+[HERE/'execution_evidence'])


def bind(reg, pin, boot_pins, deadline):
    global _bound,_deadline
    ...
    four_barrier.validate_registry(reg)
    if type(deadline) not in (int,float) or not time.monotonic()<deadline<=time.monotonic()+900:raise ValueError('complete phase deadline')
    _deadline=deadline
...
def authenticate(reg, pin, approval, approval_sha, phase, ledger):
    ...
    expected = dict(registry_sha256=pin, phase=phase, seconds=900, workers=1,
                    ...
                    failure_reserve=storage.RESERVE, inherited_certificate_registry_sha256=LEGACY_SHA, four_tree_certificate_registry_sha256=reg['four_tree']['registry_sha256'])
    ...
    certificate_chain('certificates_replay', reg['four_tree']['replay_sha256'], pin, ledger)
    return ap


def paired_certificate_chain(phase, pin, downstream_sha, ledger):
...
def names(phase):
    ...
    if phase == 'preflight':
        ...
        result.add('SCALAR_DIRECTIONS.json')
...
def attempt(out, meta, work, seconds=900):
    ...
    meta = dict(meta, start_utc=now(), inherited_certificate_registry_sha256=LEGACY_SHA,
                four_tree_certificate_registry_sha256=_bound[0]['four_tree']['registry_sha256'])
    ...
    if remaining<=0:raise TimeoutError('phase deadline exhausted before work')
    signal.setitimer(signal.ITIMER_REAL, remaining)
    try:
        outputs['ATTEMPT.json'] = save(out / 'ATTEMPT.json', meta)
        summary = work(out, ledger, outputs, _deadline)
        outputs['ACCESS.json'] = save(out / 'ACCESS.json', ledger)
        _budget.verify()
        result = dict(meta, status='COMPLETE', finish_utc=now(), seconds=time.monotonic() - (_deadline-900),
...
def certificate_chain(phase,pin,downstream_sha,ledger):
    if _bound is None or phase!="certificates_replay" or downstream_sha!=_bound[1]:raise ValueError("four-tree boundary")
    reg=_bound[0]
    if pin!=reg["four_tree"]["replay_sha256"]:raise ValueError("four-tree replay predecessor")
    paired=paired_certificate_chain(phase,reg["inherited"]["replay_sha256"],downstream_sha,ledger)
    return four_barrier.authenticate(reg,ROOT,read,json_read,ledger,CONTEXTS,paired,modules)
