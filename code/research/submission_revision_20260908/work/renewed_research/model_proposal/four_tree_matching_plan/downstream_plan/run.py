...
LEGACY = HERE.parents[1] / 'paired_tree_plan/all_context_plan'
...
LOCAL = ('run.py','storage.py','four_barrier.py','adapter.py','study_inputs.py','legacy_results.py','study.py')
...
def execute_bounded(args,deadline):
    ...
    with loaded({name: (p, buffers[p]) for name, p in [
        ...
        ('four_barrier', HERE / 'four_barrier.py'), ('adapter', HERE / 'adapter.py'),
        ('study_inputs', HERE / 'study_inputs.py'), ('legacy_results', HERE / 'legacy_results.py'),
        ('downstream_study', HERE / 'study.py')]
    }) as ms:
        a = ms['adapter']
        a.bind(reg, args.registry_sha256, pins, deadline)
