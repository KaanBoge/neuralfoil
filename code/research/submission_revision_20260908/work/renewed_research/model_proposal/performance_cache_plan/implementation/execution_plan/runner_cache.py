...
import argparse,datetime,hashlib,json,math,os,platform,resource,signal,subprocess,sys,time,traceback,types
...
def authorize(args):
    ...
    a=json.loads(ab)
    required={'authorized_phase','registry_sha256','host_other_scientific_jobs_stopped','output','runtime','max_seconds','max_request_seconds','max_rss_bytes','max_output_bytes','routes','schedule','cold_repeats','warmups','warm_repeats','workers','reference_contract','optimization_contract'}
    if set(a)!=required or a['authorized_phase']!='one_fixed_request_local_benchmark_v1' or a['host_other_scientific_jobs_stopped'] is not True:raise ValueError('approval schema/scope')
    rb=raw(H/'REGISTRY.json')
    if sha(rb)!=a['registry_sha256']:raise ValueError('registry identity')
    r=json.loads(rb)
    for k in ['runtime','max_seconds','max_request_seconds','max_rss_bytes','max_output_bytes','routes','schedule','cold_repeats','warmups','warm_repeats','workers','reference_contract','optimization_contract']:
    ...
    sources={n:raw(H/r['source_paths'][n]) for n in r['sources']}
    for n,b in sources.items():
        if sha(b)!=r['sources'][n]:raise ValueError('source changed '+n)
    return a,r,sources,ab,rb
def execute_sources(sources,r):
    saved={};events=[]
    try:
        for name,file in [('provenance_v2','provenance_v2.py'),('serving','serving.py'),('timing','timing.py'),('oracle_v5','oracle_v5.py'),('loader_v5','loader_v5.py'),('measurement_v5','measurement_v5.py'),('watchdog_v6','watchdog_v6.py'),('response_snapshot','response_snapshot.py'),('feature_adapter','feature_adapter.py'),('serving_optimized','serving_optimized.py'),('preflight','preflight_cache.py'),('loader_cache','loader_cache.py')]:
            saved[name]=sys.modules.get(name);m=types.ModuleType(name);m.__file__=str(H/r['source_paths'][file]);sys.modules[name]=m
            exec(compile(sources[file],str(H/r['source_paths'][file]),'exec'),m.__dict__)
            events.append({'operation':'source execution','file':str(H/r['source_paths'][file]),'sha256':sha(sources[file])})
...
def finish_bootstrap(args,a,r,sources,ab,rb,events):
    if raw(args.approval)!=ab or raw(H/'REGISTRY.json')!=rb:raise ValueError('approval/registry changed')
    for n,b in sources.items():
        if raw(H/r['source_paths'][n])!=b:raise ValueError('source changed '+n)
    events.extend([{'operation':'finish source authentication','file':str(H/r['source_paths'][n]),'sha256':sha(b)} for n,b in sources.items()])
    events.extend([{'operation':'finish approval authentication','file':args.approval,'sha256':sha(ab)},{'operation':'finish registry authentication','file':str(H/'REGISTRY.json'),'sha256':sha(rb)}])
def child(args,a,r,sources,ab,rb,events):
    import numpy as np
    from loader_cache import load,finish,references as old_references,authenticate_legacy
    from measurement_v5 import measured,warm
    from preflight import preflight as full_preflight
    ...
    try:
        ...
        authenticate_legacy(r,access)
        serv,refs,runtime,access,runtime_pins,original_service=load(args.project,r,route=route,with_references=args.phase=='preflight',access=access)
        ...
        if args.phase=='preflight':
            # Whole untimed phase, including all independent comparisons, has the
            # unchanged 60-second ceiling. No nested per-request alarm may reset it.
            serv.request=original
            values=old_references(r,access)
            fidelity=full_preflight(original_service,serv,values,refs)
            archive_inventory=fidelity['archive_inventory']
            p=out/'COMPONENT_FIDELITY.json';write(p,fidelity);outputs[p.name]=sha(raw(p))
...
def validate_child_receipt(c,phase,route,predecessor,approval_sha,registry_sha,registry):
    if (c.get('status')!='PASS' or c.get('phase')!=phase or c.get('route')!=route or
        c.get('predecessor_sha256')!=predecessor or c.get('approval_sha256')!=approval_sha or
        c.get('registry_sha256')!=registry_sha or c.get('reference_contract')!=registry['reference_contract'] or c.get('optimization_contract')!=registry['optimization_contract']):
        raise ValueError('child phase/route/predecessor/source contract binding')
    peak=c.get('self_ru_maxrss_bytes')
    if (type(peak) not in (int,float) or not math.isfinite(peak) or peak<=0 or
        peak>registry['max_rss_bytes'] or c.get('ru_maxrss_units')!='bytes on explicitly required macOS'):
        raise ValueError('child self RSS missing/invalid/over cap or wrong units')

def parent(args,a,r,sources,ab,rb,events):
    from watchdog_v6 import monitor,budget
    ...
    write(out/'ATTEMPT.json',{'approval_sha256':sha(ab),'registry_sha256':sha(rb),'started_UTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),'load_average':os.getloadavg(),'platform':platform.platform(),'schedule':r['schedule'],'rss_method':'direct-child macOS ps KiB at100ms plus authenticated positive child self peak; bounded terminal observation, live-unavailable fails closed','host_quiet_assertion':a['host_other_scientific_jobs_stopped']})
    ...
    try:
        for phase,route in [('preflight',None)]+[('cold',x) for x in r['routes']]+[('warm',None)]:
            ...
            command=[sys.executable,str(H/'runner_cache.py'),'--project',args.project,'--approval',args.approval,'--approval-sha256',args.approval_sha256,'--output',args.output,'--phase',phase]
            ...
            validate_child_receipt(c,phase,route,predecessor,sha(ab),sha(rb),r)
            accept(receipt)
        ...
        write(pending,{'optimization_contract':r['optimization_contract'],'reference_contract':r['reference_contract'],'status':'COMPLETE_REQUIRES_INDEPENDENT_REVIEW','elapsed_seconds':time.monotonic()-start,'approval_sha256':sha(ab),'registry_sha256':sha(rb),'preflight_sha256':predecessor,'warm_records':49,'cold_records':7,'untimed_fidelity_requests':7,'untimed_warmup_requests':14,'events':events,'load_average_end':os.getloadavg(),'outputs':accepted})
        check_budget()
...
def main():
    ...
    saved={}
    try:
        def timeout(*unused):raise TimeoutError('request or complete preflight phase budget')
        signal.signal(signal.SIGALRM,timeout)
        if args.phase=='preflight':signal.alarm(r['max_request_seconds'])
        saved,events=execute_sources(sources,r)
        events[:0]=[{'operation':'source file read','file':str(H/r['source_paths'][n]),'sha256':sha(b)} for n,b in sources.items()]+[{'operation':'JSON parse','file':args.approval,'sha256':sha(ab)},{'operation':'JSON parse','file':str(H/'REGISTRY.json'),'sha256':sha(rb)}]
        if args.phase=='parent':parent(args,a,r,sources,ab,rb,events)
        else:child(args,a,r,sources,ab,rb,events)
    finally:signal.alarm(0);restore(saved)
