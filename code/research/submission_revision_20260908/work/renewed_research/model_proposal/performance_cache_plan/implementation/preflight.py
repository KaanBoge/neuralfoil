...
def preflight(original,optimized,frozen_v6,original_archive_refs):
    ...
    original_work={c:{k:ResponseSnapshot({'v':v}) for k,v in w.items() if isinstance(v,np.ndarray)} for c,w in original.workload.items()}
    optimized_work={c:{k:ResponseSnapshot({'v':v}) for k,v in w.items() if isinstance(v,np.ndarray)} for c,w in optimized.workload.items()}
    for c in cohorts:
    ...
    for service,before in [(original,original_work),(optimized,optimized_work)]:
        for c,w in before.items():
            for k,v in w.items():same(service.workload[c][k],v.copy()['v'],'caller input mutation')
