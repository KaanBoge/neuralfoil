...
class Serving:
    ...
    def features(self,w):
        ...
        for name,text in w['coordinates'].items():
            ...
            foil=self.asb.Airfoil(name='shape_sensitivity',coordinates=coords).to_kulfan_airfoil()
            kp=foil.kulfan_parameters
            k18[ix]=np.r_[kp['upper_weights'],kp['lower_weights'],kp['leading_edge_weight'],kp['TE_thickness']]
            sensitivity,retained=calc_with_foil(coords,w['alpha'][ix],w['Re'][ix],foil,f)
            for k in mat:mat[k][ix]=sensitivity[k]
            base=predict_base_with_foil(coords,w['alpha'][ix],w['Re'][ix],foil,retained,f)
