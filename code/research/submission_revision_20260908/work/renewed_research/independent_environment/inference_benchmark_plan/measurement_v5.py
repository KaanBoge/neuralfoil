...
import time
import numpy as np
from serving import ROUTES,COHORTS,QUALIFIED
from oracle_v5 import exact, same, inventory
    ...
    for route in ROUTES:
        ...
        for c in COHORTS:
            ...
            if route in ROUTES[:2]:same(d[c+'/quantized_CD'],r['features']['XLARGE_CD' if route==ROUTES[0] else 'BASE_CD'],'baseline')
                ...
                if route==ROUTES[2]:
                    oracle=r['oracle'].evaluate({k:d[c+'/'+k] for k in ['X62','BASE_CD','all_model_CD']},serv.workload[c]['gate'],r['original_core'])
                    exact({k:d[c+'/'+k] for k in oracle},oracle)
                    archive_inventory[c]=inventory(oracle['CD'],r['original'])
                    # Frozen reference is independently evaluated oracle, never the candidate.
                    for k,v in oracle.items():d[c+'/'+k]=v
                else:
        ...
        result[route]={key:d[key] for key in value}
    return result,archive_inventory
