...
def authorize(args):
    ...
    if set(a)!=required or a['authorized_phase']!='one_fixed_inference_benchmark_v3' or a['host_other_scientific_jobs_stopped'] is not True:raise ValueError('approval schema/scope')
    rb=raw(H/'REGISTRY_v3.json')
...
def execute_sources(sources):
    saved={};events=[]
    try:
        for name,file in [('provenance_v2','provenance_v2.py'),('serving','serving.py'),('timing','timing.py'),('loader_v2','loader_v2.py'),('measurement_v3','measurement_v3.py'),('watchdog_v3','watchdog_v3.py')]:
...
def finish_bootstrap(args,a,r,sources,ab,rb,events):
    if raw(args.approval)!=ab or raw(H/'REGISTRY_v3.json')!=rb:raise ValueError('approval/registry changed')
    for n,b in sources.items():
    ...
    events.extend([{'operation':'finish approval authentication','file':args.approval,'sha256':sha(ab)},{'operation':'finish registry authentication','file':str(H/'REGISTRY_v3.json'),'sha256':sha(rb)}])
def child(args,a,r,sources,ab,rb,events):
    import numpy as np
    from loader_v2 import load,finish
    from measurement_v3 import preflight,measured,warm
...
def parent(args,a,r,sources,ab,rb,events):
    ...
    try:
        for phase,route in [('preflight',None)]+[('cold',x) for x in r['routes']]+[('warm',None)]:
            ...
            command=[sys.executable,str(H/'runner_v3.py'),'--project',args.project,'--approval',args.approval,'--approval-sha256',args.approval_sha256,'--output',args.output,'--phase',phase]
        ...
        write(pending,{'status':'COMPLETE_REQUIRES_INDEPENDENT_REVIEW','elapsed_seconds':time.monotonic()-start,'approval_sha256':sha(ab),'registry_sha256':sha(rb),'preflight_sha256':predecessor,'warm_records':49,'cold_records':7,'untimed_fidelity_requests':7,'untimed_warmup_requests':14,'events':events,'load_average_end':os.getloadavg(),'outputs':accepted})
        check_budget()
...
def main():
    ...
    try:
        ...
        events[:0]=[{'operation':'source file read','file':str(H/n),'sha256':sha(b)} for n,b in sources.items()]+[{'operation':'JSON parse','file':args.approval,'sha256':sha(ab)},{'operation':'JSON parse','file':str(H/'REGISTRY_v3.json'),'sha256':sha(rb)}]
