...
def authorize(args):
    ...
    if set(a)!=required or a['authorized_phase']!='one_fixed_inference_benchmark_v4' or a['host_other_scientific_jobs_stopped'] is not True:raise ValueError('approval schema/scope')
    rb=raw(H/'REGISTRY_v4.json')
...
def finish_bootstrap(args,a,r,sources,ab,rb,events):
    if raw(args.approval)!=ab or raw(H/'REGISTRY_v4.json')!=rb:raise ValueError('approval/registry changed')
    for n,b in sources.items():
    ...
    events.extend([{'operation':'finish approval authentication','file':args.approval,'sha256':sha(ab)},{'operation':'finish registry authentication','file':str(H/'REGISTRY_v4.json'),'sha256':sha(rb)}])
...
def parent(args,a,r,sources,ab,rb,events):
    ...
    try:
        for phase,route in [('preflight',None)]+[('cold',x) for x in r['routes']]+[('warm',None)]:
            ...
            command=[sys.executable,str(H/'runner_v4.py'),'--project',args.project,'--approval',args.approval,'--approval-sha256',args.approval_sha256,'--output',args.output,'--phase',phase]
...
def main():
    ...
    try:
        ...
        events[:0]=[{'operation':'source file read','file':str(H/n),'sha256':sha(b)} for n,b in sources.items()]+[{'operation':'JSON parse','file':args.approval,'sha256':sha(ab)},{'operation':'JSON parse','file':str(H/'REGISTRY_v4.json'),'sha256':sha(rb)}]
