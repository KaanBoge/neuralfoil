...
def authorize(args):
    ...
    a=json.loads(ab)
    required={'authorized_phase','registry_sha256','host_other_scientific_jobs_stopped','output','runtime','max_seconds','max_request_seconds','max_rss_bytes','max_output_bytes','routes','schedule','cold_repeats','warmups','warm_repeats','workers'}
    if set(a)!=required or a['authorized_phase']!='one_fixed_inference_benchmark_v2' or a['host_other_scientific_jobs_stopped'] is not True:raise ValueError('approval schema/scope')
    rb=raw(H/'REGISTRY_v2.json')
...
def execute_sources(sources):
    saved={};events=[]
    try:
        for name,file in [('provenance_v2','provenance_v2.py'),('serving','serving.py'),('timing','timing.py'),('loader_v2','loader_v2.py'),('measurement_v2','measurement_v2.py'),('watchdog_v2','watchdog_v2.py')]:
...
def finish_bootstrap(args,a,r,sources,ab,rb,events):
    if raw(args.approval)!=ab or raw(H/'REGISTRY_v2.json')!=rb:raise ValueError('approval/registry changed')
    for n,b in sources.items():
    ...
    events.extend([{'operation':'finish approval authentication','file':args.approval,'sha256':sha(ab)},{'operation':'finish registry authentication','file':str(H/'REGISTRY_v2.json'),'sha256':sha(rb)}])
def child(args,a,r,sources,ab,rb,events):
    import numpy as np
    from loader_v2 import load,finish
    from measurement_v2 import preflight,measured,warm
    ...
    try:
        ...
        if args.phase=='preflight':
            values=preflight(serv,refs)
            for route,v in values.items():
                p=out/('reference_'+route+'.npz')
...
def parent(args,a,r,sources,ab,rb,events):
    from watchdog_v2 import monitor
    ...
    out.mkdir();start=time.monotonic();processes=[];predecessor=None
    ...
    try:
        for phase,route in [('preflight',None)]+[('cold',x) for x in r['routes']]+[('warm',None)]:
            ...
            command=[sys.executable,str(H/'runner_v2.py'),'--project',args.project,'--approval',args.approval,'--approval-sha256',args.approval_sha256,'--output',args.output,'--phase',phase]
            ...
            with (out/(identity+'.stdout')).open('x') as so,(out/(identity+'.stderr')).open('x') as se:
                ...
                result=monitor(proc,start,r['max_seconds'],r['max_rss_bytes'],lambda:sum(p.stat().st_size for p in out.iterdir() if p.is_file()),r['max_output_bytes'])
            ...
            for n,h in c['outputs'].items():
                if sha(raw(out/n))!=h:raise ValueError('child output changed')
        ...
        for n in final_access.spec['files']:final_access.file(n)
        for archive,spec in final_access.spec['archives'].items():
            for member in spec['members']:final_access.member(archive,member)
        events.extend(final_access.events)
        finish_bootstrap(args,a,r,sources,ab,rb,events)
        write(out/'COMPLETE.json',{'status':'COMPLETE_REQUIRES_INDEPENDENT_REVIEW','elapsed_seconds':time.monotonic()-start,'approval_sha256':sha(ab),'registry_sha256':sha(rb),'preflight_sha256':predecessor,'warm_records':49,'cold_records':7,'events':events,'load_average_end':os.getloadavg(),'outputs':{p.name:sha(raw(p)) for p in out.iterdir() if p.is_file()}})
    except BaseException as e:
...
def main():
    ...
    try:
        ...
        events[:0]=[{'operation':'source file read','file':str(H/n),'sha256':sha(b)} for n,b in sources.items()]+[{'operation':'JSON parse','file':args.approval,'sha256':sha(ab)},{'operation':'JSON parse','file':str(H/'REGISTRY_v2.json'),'sha256':sha(rb)}]
