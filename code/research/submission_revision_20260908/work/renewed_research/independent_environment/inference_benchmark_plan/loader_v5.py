...
from oracle_v5 import same, CanonicalOracle
...
def load(root,registry,route=None,with_references=False,access=None):
    ...
    with modules(access) as execute:
        ...
        if want_original:
            ...
            artifact=access.json(access.file(identity),identity)
            original=prep.prepare(artifact)
            if with_references:
                portable=execute('bench_canonical_portable_v5',access.file(ORIGINAL+'predictor.py'),ORIGINAL+'predictor.py')
                native_policy_path='model_development_20260907_risk_policy/risk_policy.py'
                native_policy=execute('bench_native_policy_v5',access.file(native_policy_path),native_policy_path)
                oracle=CanonicalOracle(portable.predict,native_policy.predict,artifact)
        if want_qualified:
        ...
        for c in COHORTS:
            ...
            if want_original:
                ...
                if with_references:keys += [c+'_'+k for k in ['unpenalized_transfer','BASE_CD','X62','all_model_CD','CORE_CD']]
            ...
            if with_references:
                ...
                references[c]={'features':ref,'original':old[c+'_unpenalized_transfer'],'original_core':old[c+'_CORE_CD'],'oracle':oracle,'native':z,'kl':kl}
