...
def authorize(args):
    ...
    if set(a)!=required or a['authorized_phase']!='one_fixed_inference_benchmark_v6' or a['host_other_scientific_jobs_stopped'] is not True:raise ValueError('approval schema/scope')
    rb=raw(H/'REGISTRY_v6.json')
...
def execute_sources(sources):
    saved={};events=[]
    try:
        for name,file in [('provenance_v2','provenance_v2.py'),('serving','serving.py'),('timing','timing.py'),('oracle_v5','oracle_v5.py'),('loader_v5','loader_v5.py'),('measurement_v5','measurement_v5.py'),('watchdog_v6','watchdog_v6.py')]:
...
def finish_bootstrap(args,a,r,sources,ab,rb,events):
    if raw(args.approval)!=ab or raw(H/'REGISTRY_v6.json')!=rb:raise ValueError('approval/registry changed')
    for n,b in sources.items():
    ...
    events.extend([{'operation':'finish approval authentication','file':args.approval,'sha256':sha(ab)},{'operation':'finish registry authentication','file':str(H/'REGISTRY_v6.json'),'sha256':sha(rb)}])
...
def validate_child_receipt(c,phase,route,predecessor,approval_sha,registry_sha,registry):
    if (c.get('status')!='PASS' or c.get('phase')!=phase or c.get('route')!=route or
        ...
        c.get('registry_sha256')!=registry_sha or c.get('reference_contract')!=registry['reference_contract']):
...
def parent(args,a,r,sources,ab,rb,events):
    ...
    try:
        for phase,route in [('preflight',None)]+[('cold',x) for x in r['routes']]+[('warm',None)]:
            ...
            command=[sys.executable,str(H/'runner_v6.py'),'--project',args.project,'--approval',args.approval,'--approval-sha256',args.approval_sha256,'--output',args.output,'--phase',phase]
...
def main():
    ...
    try:
        ...
        events[:0]=[{'operation':'source file read','file':str(H/n),'sha256':sha(b)} for n,b in sources.items()]+[{'operation':'JSON parse','file':args.approval,'sha256':sha(ab)},{'operation':'JSON parse','file':str(H/'REGISTRY_v6.json'),'sha256':sha(rb)}]
