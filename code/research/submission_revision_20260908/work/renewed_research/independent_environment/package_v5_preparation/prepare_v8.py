...
def permitted(name):
    ...
    if any(x.startswith(('venv','conda')) or x in {'.venv','.git','__pycache__','site-packages','pkgs','envs','.cache','cache'} for x in parts):raise ValueError('environment/cache scope')
...
def prepare(root,approval,approval_sha,output):
    ...
    add(Path(__file__),'authoring_provenance/prepare_v8.py','inventory_builder',a['builder_sha256'])
