...
from engine_v3 import Limits,refine
...
def main():
    ...
    out={'fixture':'400 deterministic15-node synthetic trees; one attempted split','elapsed_seconds':elapsed,
         ...
         'source_sha256':{str(p.relative_to(ROOT)):sha(p) for p in (Path(__file__).resolve(),ROOT/'pilot_engine_v2.py',ROOT/'PILOT_REGISTRY_V2.json',Path(__file__).resolve().parent/'engine_v3.py',Path(__file__).resolve().parent/'cache_v3.py')}}
    exclusive_json(Path(__file__).resolve().parent/'PROFILE_V3_RESULT.json',out)
