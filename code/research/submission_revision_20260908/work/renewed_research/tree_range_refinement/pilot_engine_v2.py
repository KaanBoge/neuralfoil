...
class Limits:
    def __init__(self, stages, seconds=120, visits=4000000, memory=128*1024*1024):
        ...
        if any(t.flags.writeable for t in self.stages):
            raise ValueError("immutable read-only stages required")
        # Retain the original root accounting tuple's entire container charge;
        # the dynamic pair gets its own charge, so decomposition cannot omit it.
    ...
    def check(self, transient=(), visit=False):
        ...
        estimate = 3 * (self.static_owned_size + owned_size((self.persistent, transient))) + 8*1024*1024
...
def bound(stages, initial, box, enclosure, limits):
    ...
    return {'box': box, 'lower': str(result['lower']), 'upper': str(result['upper']),
...
def refine(stages, initial, enclosure, limits, publish, splits=128, dimensions=62):
    ...
    for _ in range(splits):
        ...
        if not choices:
            return current, 'routing_resolved'
        ...
        current = candidate
        limits.completed_splits = current['splits']
        limits.persistent = current
        if current['splits'] in (1, 4, 16, 64, 128):
            publish(current)
    return current, ('routing_resolved' if not any(x['cuts'] for x in current['frontier']) else 'split_budget')
