...
import pilot_engine_v2 as engine
...
class Tests(unittest.TestCase):
    ...
    def test_engine_durable_json_parse_independent_checker(self):
        with tempfile.TemporaryDirectory() as d:
            paths=[]
            def publish(state):
                p=Path(d)/('checkpoint_%03d.json'%state['splits'])
                exclusive_json(p,state)
                paths.append(p)
            final,reason=engine.refine(self.model.stages,0.,Q.sequential_range,
                         engine.Limits(self.model.stages),publish,splits=2)
            self.assertEqual(reason,'routing_resolved')
            self.assertEqual(final['splits'],1)
            self.assertEqual(len(paths),2)
            for p in paths:
                state=json.loads(p.read_text())
                self.assertTrue(all(type(row['lower']) is str and type(row['upper']) is str for row in state['frontier']))
                checker.replay(state,self.model.stages,0.,Q.sequential_range)

    def test_interrupt_during_actual_checkpoint_serialization(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d)/'actual.json'
            states=self.states()
            old=Path(d)/'old.json';exclusive_json(old,states[0]);before=adapter.sha(old)
            chunks=[]
            def interrupt(chunk):
                chunks.append(chunk)
                if len(chunks)==20:raise KeyboardInterrupt('mid serialization')
            with self.assertRaises(KeyboardInterrupt):
                exclusive_json(p,states[-1],serialization_interrupt=interrupt)
            self.assertFalse(p.exists())
            self.assertGreater(Path(str(p)+'.partial').stat().st_size,0)
            self.assertEqual(adapter.sha(old),before)
            checker.replay(json.loads(old.read_text()),self.model.stages,0.,Q.sequential_range)

    def test_completed_split_and_stop_reason(self):
        limit=engine.Limits(self.model.stages)
        state,reason=engine.refine(self.model.stages,0.,Q.sequential_range,limit,lambda x:None,splits=0)
        self.assertEqual(reason,'split_budget');self.assertEqual(limit.completed_splits,0)
        self.assertTrue(limit.initialized)

    def test_static_cache_requires_read_only_arrays(self):
        with self.assertRaises(ValueError):engine.Limits([stump()])

    def test_cached_estimate_is_not_weaker(self):
        from pilot_engine import Limits as OldLimits
        from pilot_engine import owned_size
        old=OldLimits(tuple(self.model.stages));new=engine.Limits(tuple(self.model.stages))
        state=self.states()[0]
        old.persistent=state;new.persistent=state
        transient=([1.,2.],{'same':state})
        old.check(transient);new.check(transient)
        self.assertGreaterEqual(new.peak_estimate,old.peak_estimate)
        self.assertEqual(new.static_owned_size,owned_size(tuple(self.model.stages))+sys.getsizeof((None,None,None)))
