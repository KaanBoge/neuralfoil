...
def exclusive_json(path, value, interrupt=None):
...
def worker(args, r, root):
    from pilot_engine import Limits, refine
    ...
    try:
        refine(model.stages, model.initial, q.sequential_range, limits, publish)
    ...
    exclusive_json(Path(args.output)/'worker_status.json', {
        'status': status, 'node_visits': limits.visits,
