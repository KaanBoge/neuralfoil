...
from domain import stamp, assert_binary64
...
def worker(args, r, root):
    from engine import Limits, refine
    model = adapter.load_model(r, root)
    q = adapter.source_module(r, root, 'qualified_numerics')
    assert_binary64()
    oracle = adapter.source_module(r, root, 'root_exact_oracle')
    limits = Limits(model.stages, seconds=120)
    def publish(state):
        exclusive_json(Path(args.output)/('candidate_%03d.json' % state['splits']), state)
    try:
        final_state, stop_reason = refine(model.stages, model.initial, q.sequential_range, limits, publish, oracle=oracle.feasible_witness)
    ...
    exclusive_json(Path(args.output)/'worker_status.json', {
        ...
        'node_visits': limits.visits, 'oracle_calls': limits.oracle_calls,
        'infeasible_queries': limits.infeasible_queries, 'domain': stamp(),
...
def check(args, r, root):
    replay = adapter.source_module(r, root, 'relation_checker').replay
    model = adapter.load_model(r, root)
    q = adapter.source_module(r, root, 'qualified_numerics')
...
def inherited(r, root):
    ...
    result.update(domain=stamp(), requested_domain=stamp(), bound_valid_on='FULL_FINITE_X62')
    return result
...
def orchestrate(args, r, root):
    ...
    exclusive_json(out/'checkpoint_000_inherited.json', inherited(r, root))
    exclusive_json(out/'start.json', {'registry_sha256': args.approved_registry_sha256,
                   'context': r['scope'], 'domain': stamp(), 'pid': os.getpid(), 'search_seconds_max': 120,
    ...
    result = inherited(r, root)
    if verified.exists() and verification and verification['returncode'] == 0:
        value = json.loads(verified.read_text())
        if value['candidate_sha256'] != adapter.sha(latest) or value['model_sha256'] != r['input']['sha256'] or value['registry_sha256'] != args.approved_registry_sha256 or value['status'] != 'PASS_RESTRICTED_DOMAIN_REPLAY':
    ...
    result.update(search=search, verification=verification, seconds=time.monotonic()-start,
                  scope_note='Label-free enclosure restricted to stamped R; runtime guard and identity fallback required. No predictive improvement, calibration or outcomes.')
