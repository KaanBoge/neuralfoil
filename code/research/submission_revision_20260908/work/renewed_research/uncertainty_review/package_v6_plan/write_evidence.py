...
class CappedFile:
    ...
    def write(self,b):
        ...
        n=self.raw.write(b)
        if n!=len(b):raise OSError('short archive write')
        self.highwater=max(self.highwater,end);return n
...
def run(root,approval,approval_sha,output):
    ...
    with c.deadline() as clock:
        try:
            ...
            for r in rows:
                if r['source'] in rd.pins and rd.pins[r['source']]!=r['sha256']:raise ValueError('payload conflicts with authenticated source role')
            ...
            try:
                ...
                with zipfile.ZipFile(capped,'w',compression=zipfile.ZIP_DEFLATED,compresslevel=6,allowZip64=True) as z:
                    ...
                    for r in rows:
                        ...
                        if r['source'] in rd.pins and rd.pins[r['source']]!=r['sha256']:raise ValueError('immutable first source pin')
