...
Only the explicitly pinned external-readonly package_v6 role validator is executed.
"""
...
TOOL='submission_revision_20260908/work/renewed_research/uncertainty_review/package_v6_plan/package_v6.py'
TOOL_SHA='f5f338ecdef3b2808993b0e152004a261c017cac580be586bdf126d85ac32057'
...
REQUIRED_SOURCES={FINALIZER_DIR+n for n in ['finalize_v9_v2.py','test_finalize_v9_v2.py','prepare_v9_final_config.py','test_prepare_v9_final_config.py']}|{EDITION+'/'+n for n in ['assemble_submission.py','test_assemble_submission.py','build_documents.py','render_version.py','audit_layout.py']}
...
def qa_paths(root,qa,config):
    """Canonical explicit external pins remain provenance, never copy candidates."""
    root=Path(root).resolve();external=qa.get('external_readonly_files')
    if type(external) is not dict or external!=config.get('external_readonly_pins'):raise ValueError('external QA/config exact inventory')
    for n,h in external.items():
        pin(h);p=Path(n)
        if not p.is_absolute() or str(p.resolve())!=n or p.is_relative_to(root):raise ValueError('canonical external identity')
        if any(x.is_symlink() for x in [p,*p.parents]):raise ValueError('external symlink')
    qmap={};seen={}
    for n,h in qa['files'].items():
        pin(h);q=root/EDITION/n
        if any(x.is_symlink() for x in [q,*q.parents]):raise ValueError('QA symlink')
        q=q.resolve()
        if not q.is_relative_to(root):
            if external.get(str(q))!=h:raise ValueError('undeclared external QA file')
            seen[str(q)]=h;continue
        key=q.relative_to(root).as_posix()
        if key in qmap and qmap[key][1]!=h:raise ValueError('conflicting QA aliases')
        qmap[key]=(n,h)
    if seen!=external:raise ValueError('external QA missing or extra')
    return qmap,external
def prepare(root,approval,approval_sha,output):
    ...
    qmap,external=qa_paths(rd.root,qa,c)
    ...
    toolraw=rd.check(TOOL,TOOL_SHA,True);tool=types.ModuleType('pinned_package_v6');exec(compile(toolraw,TOOL,'exec'),tool.__dict__)
    for r in rows:
