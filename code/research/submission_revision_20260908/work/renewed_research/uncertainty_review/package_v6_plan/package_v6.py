...
def external_readonly_file(raw,h):
    """Exact hash-only external file admission; never a package source."""
    pin(h)
    if (type(raw) is not str or not raw.startswith('/') or raw!=str(Path(raw))
        or any(x in {'','.','..'} for x in raw.split('/')[1:])
        or any(ord(c)<32 or ord(c)==127 for c in raw)):
        raise ValueError('Canonical external file required')
    p=Path(raw)
    no_symlink_ancestors(p)
    if not p.is_file() or not stat.S_ISREG(p.stat().st_mode):
        raise ValueError('External regular file required')
    hashed=hashlib.sha256()
    with p.open('rb') as f:
        for block in iter(lambda:f.read(65536),b''):hashed.update(block)
    if hashed.hexdigest()!=h:raise ValueError('External hash mismatch')
    return p

def external_qa_contract(root,edition,qa):
    """Authenticate the full QA's exact map against its pinned project config."""
    ext=qa.get('external_readonly_files',{})
    if type(ext) is not dict:raise ValueError('Exact external QA map required')
    if 'external_readonly_files' not in qa:return {}
    rawpath=edition/qa['config_path']
    no_symlink_ancestors(rawpath)
    configpath=rawpath.resolve()
    if not configpath.is_relative_to(root):raise ValueError('Config outside project')
    key=Path(os.path.relpath(configpath,edition)).as_posix()
    if qa['files'].get(key)!=qa['config_sha256']:raise ValueError('Config absent from QA pins')
    config=parse(checked(configpath,qa['config_sha256']))
    if config.get('external_readonly_pins')!=ext:raise ValueError('External QA/config map mismatch')
    if config.get('finalization_authorized') is not True:raise ValueError('Config not authorized')
    for raw,h in ext.items():
        p=external_readonly_file(raw,h)
        if p.is_relative_to(root):raise ValueError('External map cannot override project')
    return ext
...
def qa_binding(root,m,buffers):
    ...
    external=external_qa_contract(root,edition,qa);seen_external={}
    for key,h in qa['files'].items():
        if '\\' in key:raise ValueError('Invalid QA path')
        resolved=(edition/key).resolve()
        if not resolved.is_relative_to(root):
            no_symlink_ancestors(edition/key)
            canonical=str(resolved)
            if external.get(canonical)!=h:raise ValueError('Unlisted external QA source')
            seen_external[canonical]=h
            continue
        pin(h);qmap[key]=(resolved,h)
    if seen_external!=external:raise ValueError('External map differs from full QA inventory')
    ...
    # Recheck the exact config and external bytes after role binding; no copying.
    if external_qa_contract(root,edition,qa)!=external:raise ValueError('External contract changed')
