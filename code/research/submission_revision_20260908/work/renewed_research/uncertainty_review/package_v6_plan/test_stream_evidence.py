...
class Tests(unittest.TestCase):
    ...
    def test_metadata_growth_after_stat(self):
        original=Path.open
        def growing(p,*args,**kwargs):
            if p==self.root/'proof.txt':return io.BytesIO(b'x'*12)
            return original(p,*args,**kwargs)
        (self.root/'proof.txt').write_bytes(b'x')
        with patch.object(c,'METADATA',8),patch.object(c,'CHUNK',4),patch.object(Path,'open',growing):
            with self.assertRaisesRegex(ValueError,'cumulative'):c.Reader(self.root,c.Clock()).check('proof.txt',c.digest(b'x'*12),True)
    ...
    def test_lying_central_count(self):
        self.create();p=self.root/'create/v9_evidence.zip';raw=bytearray(p.read_bytes())
        struct.pack_into('<HH',raw,len(raw)-14,1,1);p.write_bytes(raw)
        with self.assertRaisesRegex(ValueError,'count'):v.directory_admission(p)
    def test_short_writes(self):
        class Short(io.BytesIO):
            def write(self,b):return super().write(b[:-1])
        with self.assertRaises(OSError):w.CappedFile(Short(),c.Clock()).write(b'ab')
        with self.assertRaises(OSError):v.write_chunk(Short(),b'ab')
    def test_mutated_extracted_disk_fails(self):
        self.create();n,h=self.auth('VERIFY_EXTRACT','extract');original=v.verify_disk
        def altered(out,expected,clock):
            p=out/'payload/evidence/proof.txt';p.write_bytes(b'x'*p.stat().st_size)
            return original(out,expected,clock)
        with patch.object(v,'verify_disk',altered):
            with self.assertRaisesRegex(ValueError,'disk hash'):v.run(self.root,n,h,'extract')
        self.assertTrue((self.root/'extract/FAILURE.json').exists());self.assertFalse((self.root/'extract/COMPLETE.json').exists())
    def test_conflicting_authenticated_source_role(self):
        p=self.root/'write_evidence.py';self.sel['files']=[{'source':'write_evidence.py','target':'provenance/helper.py','sha256':'0'*64,'bytes':p.stat().st_size}];self.put('selection.json',self.sel);n,h=self.auth()
        with self.assertRaisesRegex(ValueError,'conflicts with authenticated'):w.run(self.root,n,h,'create')
        self.assertFalse((self.root/'create').exists())
