...
class Tests(unittest.TestCase):
    ...
    def test_external_exact_provenance_not_copy(self):
        with tempfile.TemporaryDirectory() as t:
            root=Path(t).resolve()/'project';root.mkdir();outside=Path(t).resolve()/'runtime.py';outside.write_text('# mock')
            h=p.digest(outside.read_bytes());ext={str(outside):h};qa={'external_readonly_files':ext,'files':{str(outside):h,'main.md':'b'*64}}
            internal,found=p.qa_paths(root,qa,{'external_readonly_pins':ext})
            self.assertEqual(found,ext);self.assertEqual(set(internal),{p.EDITION+'/main.md'})
            for bad in [{},{str(outside):'a'*64}]:
                with self.assertRaises(ValueError):p.qa_paths(root,qa,{'external_readonly_pins':bad})
            bad=copy.deepcopy(qa);bad['files']['/another/unapproved/runtime.py']='c'*64
            with self.assertRaises(ValueError):p.qa_paths(root,bad,{'external_readonly_pins':ext})
    def test_missing_external_inventory_member(self):
        with tempfile.TemporaryDirectory() as t:
            root=Path(t).resolve()/'project';root.mkdir();outside=Path(t).resolve()/'runtime.py';outside.write_text('mock');ext={str(outside):'a'*64}
            with self.assertRaises(ValueError):p.qa_paths(root,{'external_readonly_files':ext,'files':{}},{'external_readonly_pins':ext})
