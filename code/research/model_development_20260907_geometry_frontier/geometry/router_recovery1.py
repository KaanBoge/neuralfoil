...
def fit_variant(frame, panels, center, model, k18, re, alpha, component_names):
    ...
    # Compare exported normalized weights consistently; preserve parity tolerance.
    np.testing.assert_allclose(pred,x@corners.ravel(),atol=1e-9,rtol=1e-12)
    projection_drift = float(np.max(np.abs(x@corners.ravel()-x@second.x[:p])))
    assert projection_drift <= 2e-6
    model['raw_to_normalized_max_drag_count_drift'] = projection_drift
    error = np.abs(pred-y)
    scores=[]
