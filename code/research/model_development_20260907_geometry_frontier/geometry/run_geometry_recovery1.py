...
def fit_one(union,d,k18,name):
    ...
    for variant in router.VARIANTS:
        ...
        if output.exists():
            expected=read(HERE/'pre_recovery_hashes.json')['sha256'].get(str(output))
            assert expected is not None and sha(output)==expected,output
            saved=read(output)
            rows.append({'split':name,'variant':variant,'seconds':saved['seconds'],
                         'status':'preserved_original_completed_fit',
                         'inner_fitted_worst_ratio':saved['actual_worst_training_ratio']})
            continue
        tick=time.monotonic()
...
def main():
    assert OUT.is_dir()
    for name in ['resume_manifest_1.json','freeze.json','complete.json']:
        assert not (OUT/name).exists(), 'Refusing overwrite: '+name
    verify(read(HERE/'pre_recovery_hashes.json')['sha256'])
    verify(read(OUT/'manifest.json')['source_sha256'])
    try:
        union=load_union()
        paths=[Path(__file__),HERE/'router_recovery1.py',HERE/'PROTOCOL.md',HERE/'test_recovery.py',
               HERE/'RECOVERY_1.md',HERE/'pre_recovery_hashes.json',
               UNION/'run_union.py',UNION/'results/freeze.json',
               FRONTIER/'conditional/conditional_stack.py']
        hashes={str(p):sha(p) for p in paths}
        hashes.update(read(HERE/'pre_recovery_hashes.json')['sha256'])
        ...
        dump(OUT/'resume_manifest_1.json',{'source_sha256':hashes,'variants':list(router.VARIANTS),
             'components':union.COMPONENTS,'contexts':names,'workers':1,'highs_threads':1})
        logs=[]
        for name in names: logs+=fit_one(union,d,k18,name)
        expected_paths=[OUT/f'{name}_{variant}.json' for name in names for variant in router.VARIANTS]
        assert len(expected_paths)==len(set(expected_paths))==64
        models={str(p):sha(p) for p in expected_paths}
        assert len(models)==64
        verify(hashes)
        dump(OUT/'freeze.json',{'model_sha256':models,'all_64_frozen_before_exposed':True,
             'recovery_provenance':{'preserved_original_models':11,'successor_models':53,
                  'first_failed_attempt':str(OUT/'failure.json'),'recovery_note_sha256':sha(HERE/'RECOVERY_1.md')}})
        ...
        dump(OUT/'failure_recovery_1.json',{'traceback':traceback.format_exc()})
        raise
