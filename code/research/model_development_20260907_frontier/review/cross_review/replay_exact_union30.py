...
def main():
    ...
    path = FRONT/'union_certificate/exact_report.json'
    stored = json.loads(path.read_text())
    d, cols, aliases, hashes = union.get_data()
    assert cols == stored['components'] and aliases == stored['component_aliases']
    for key, values in [('P_float64_little_endian', d[cols].to_numpy()), ('y_float64_little_endian',d.measured_CD.to_numpy()), ('xlarge_CD',d.xlarge_CD.to_numpy()), ('mean8_CD',d.mean8_CD.to_numpy())]:
        assert hashlib.sha256(np.ascontiguousarray(values,dtype='<f8').tobytes()).hexdigest() == stored['actual_array_sha256'][key]
    assert hashes == stored['input_sha256']
    panels = original.panels(d)
    assert len(cols)==30 and len(panels)==23 and len(d)==29856
    ...
    report = {'scope': stored.get('scope','30 fixed gated alias components, 23 fixed retrospective panels; not universal'), 'rows':len(d),'panels':len(panels),'components':cols,'solutions':{},'input_sha256':hashes}
    for name, bases in [('both',['xlarge_CD','mean8_CD']),('xlarge',['xlarge_CD'])]:
        ...
        for row in s['exact_panel_margins']:
            assert margins[row['panel']+'|'+row['baseline']] == unpack(row['margin_fraction'])
        dual=FRONT/f"union_certificate/numeric_{name}_{s['method']}.npz"
        assert sha(FRONT/f'union_calibration/blend_{name}.json') == s['numerical']['root_blend_sha256']
    ...
    for filename,expected in stored['source_sha256'].items():assert sha(filename)==expected
    for filename,expected in hashes.items():assert sha(filename)==expected
    sources=[Path(__file__),path,FRONT/'union_certificate/certify_union.py',FRONT/'union_calibration.py',FEAS/'solve_feasibility.py']
    ...
    (HERE/'exact_union30_replay.json').write_text(json.dumps(report,indent=2)+'\n')
