"""Frozen nested capacity check with archive-matched controls; no old writes."""
...
def main():
    ...
    paths = [Path(__file__), Path(models.__file__), ROOT / "PROTOCOL.md", Path(inputs.__file__), C4 / "transition_inputs.py", C4 / "shape_inputs/manifest.json", C4 / "inputs/manifest.json"]
    ...
    manifest = {"started_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()), "hashes": hashes,
                ...
                "candidates": LABELS, "historical_rows": 8371, "groups": 93, "workers": 2,
                "status": "Adaptive capacity exploration; all fitting and selection historical only"}
