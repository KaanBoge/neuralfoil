...
CONTROLS=['project_mean8_upper_free','project_xlarge_upper_free','proper_upper_free_full','proper_upper_free_half']
LABELS=CANDIDATES+CONTROLS+REFS
...
def source_join(d):
    old=pd.read_csv(PROJECT/'model_development_20260907_cap_ablation/assessment/all_row_predictions.csv',low_memory=False)
    for split in d.split.unique():
        ...
        for key in CANDIDATES+['proper_upper_free_full','proper_upper_free_half','measured_CD']+BASES:
            np.testing.assert_allclose(f[key],fresh[key],rtol=0,atol=1e-13)
        for key in REFS+CONTROLS+['measured_CD','Re','alpha']+BASES:
...
def main():
    ...
    assert frozen['new_fit_count']==64 and frozen['external_outcomes_opened'] is False
    ...
    for seed in SEEDS:
        ...
        draws=np.random.default_rng(2026090733).multinomial(93,np.full(93,1/93),size=20000)
        ...
        for label in LABELS:
            ...
            for ref in REFS:
                ...
                compare(bt.loc[label,seed,ref],{'rows':8371,'groups':93,'remaining_MAE_reduction_percent':gain,'conditional_95pct_lower':lo,'conditional_95pct_upper':hi,'bootstrap_fraction_benefit_positive':float((benefit>0).mean()),'draws':20000,'seed':2026090733},(label,seed,ref));checks['bootstraps']+=1
    verdicts=[]
