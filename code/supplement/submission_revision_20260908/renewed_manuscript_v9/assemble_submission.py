...
def references(text,count=21):
    ...
    if ids!=list(range(1,count+1)):raise ValueError('exact ordered references')
    return section
def tables(text):return TABLE.findall(text)
def captions(text):return [m.group(0) for m in CAPTION.finditer(text)]
def preserved(old,new,expected_tables,expected_captions,reference_addendum):
    ...
    if references(old)+reference_addendum!=references(new,23):raise ValueError('old reference bytes or authorized addendum changed')
...
def compose(cfg,root):
    required={'schema','version','inputs','roles','additions','tokens','assets','evidence','reference_addendum','predecessor'}
    if set(cfg)!=required or cfg['schema']!='V9_FINITE_ASSEMBLY_V2' or not re.fullmatch(r'v[1-9][0-9]*',cfg['version']):raise ValueError('strict config schema/version')
    ...
    used=set(cfg['roles'].values())|set(cfg['additions'])|set(cfg['assets'])|set(cfg['evidence'].values())|{v['input'] for v in cfg['tokens'].values()}|{cfg['reference_addendum']}
    ...
    reference_addendum=texts[cfg['reference_addendum']]
    if [int(v) for v in re.findall(r'^\[(\d+)\]',reference_addendum,re.M)]!=[22,23] or '# References' in reference_addendum:raise ValueError('exact two appended references')
    ...
    audit={'main':preserved(texts[cfg['roles']['v8_main']],main,4,4,reference_addendum),'supplement':preserved(texts[cfg['roles']['v8_supplement']],supp,36,35,reference_addendum)}
    ...
    report['counts']['references_each']=23
    report['reference_addendum_sha256']=sha(reference_addendum.encode())
