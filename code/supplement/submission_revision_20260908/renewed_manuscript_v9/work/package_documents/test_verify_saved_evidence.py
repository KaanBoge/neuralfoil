...
class Tests(unittest.TestCase):
    ...
    def test_reserved_manifest_cannot_count_as_payload(self):
        m = copy.deepcopy(self.manifest)
        m['files'] = {'MANIFEST.json': {'bytes': 0, 'sha256': '0' * 64}}
        a, h = self.create(m, rows=[])
        with self.assertRaisesRegex(ValueError, 'Exact manifest membership'):
            v.verify(self.archive, a, h)

    def test_archive_symlink_swap_before_final_hash(self):
        a, m = self.create()
        same = self.root / 'same.zip'
        same.write_bytes(self.archive.read_bytes())
        ...
        def swap(*args):
            ...
            if len(calls) == 3:
                self.archive.unlink()
                self.archive.symlink_to(same)
            return result
        with patch.object(v, 'stream_hash', swap):
