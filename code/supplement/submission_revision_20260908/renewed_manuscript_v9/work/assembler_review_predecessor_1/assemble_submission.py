...
def compose(cfg,root):
    ...
    pngs={p.name for p in assetpaths if p.suffix=='.png'}
    image_audit={}
    for kind,text in [('main',main),('supplement',supp)]:
        links=re.findall(r'!\[[^\]]*\]\(([^)]+)\)',text)
        if len(links)!=(4 if kind=='main' else 2):raise ValueError('figure link count')
        for value in links:
            if not value.startswith('figures/') or Path(value).name not in pngs or str(PurePosixPath(value))!=value or '..' in PurePosixPath(value).parts:raise ValueError('unresolved figure alias')
