...
def fixture(root):
    ...
    extra='[22] Added primary reference 22.\n\n[23] Added primary reference 23.\n'
    put('reference_addendum',v9/'additions/reference_addendum.md',extra)
    put('main',v9/'manuscript.md',main+extra)
    put('supplement',v9/'supplement.md',supp+extra)
    ...
    cfg={'schema':'V9_FINITE_ASSEMBLY_V2','version':'v1','inputs':inputs,'roles':{k:k for k in ['main','supplement','v8_main','v8_supplement']},'additions':['refined','runtime'],'tokens':tokens,'assets':assets,'evidence':evidence,'reference_addendum':'reference_addendum','predecessor':None}
    return cfg,root/v9
...
class Tests(unittest.TestCase):
    ...
    def test_appendix_reference_not_exact_authorized_bytes_rejected(self):
        self.rewrite('main',lambda s:s.replace('Added primary reference 22.','Unauthorized replacement.'))
        with self.assertRaisesRegex(ValueError,'reference bytes'):a.compose(self.cfg,self.root)
    def test_missing_and_duplicate_reference_ids_rejected(self):
        self.rewrite('supplement',lambda s:s.replace('[23] Added primary reference 23.','[22] Added primary reference 23.'))
        with self.assertRaisesRegex(ValueError,'ordered references'):a.compose(self.cfg,self.root)
    def test_bad_reference_addendum_scope_rejected(self):
        self.rewrite('reference_addendum',lambda s:s+'[24] Unapproved third addition.\n')
        with self.assertRaisesRegex(ValueError,'two appended references'):a.compose(self.cfg,self.root)
    ...
    def test_same_basename_wrong_full_image_path_rejected(self):
        self.rewrite('main',lambda s:s.replace('figures/figure_1.png','figures/sub/figure_1.png'))
        with self.assertRaisesRegex(ValueError,'unresolved figure alias'):a.compose(self.cfg,self.root)
