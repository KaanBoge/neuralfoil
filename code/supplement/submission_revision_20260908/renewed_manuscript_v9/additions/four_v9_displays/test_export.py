...
class Tests(unittest.TestCase):
    ...
    def test_full21_summary_source_selects_only_two(self):
        s,t,p=fixture();rows=e.csv_rows(t['candidate_summary.csv'])
        rows.extend(dict(candidate='archived_control_'+str(i),minimum='-9.123456789') for i in range(19))
        t['candidate_summary.csv']=e.csv_bytes(rows);out=e.prepare(s,t,p)
        self.assertEqual(len(e.csv_rows(out['candidate_summary.csv'])),2)
        self.assertEqual(len(e.csv_rows(out['source_candidate_summary.csv'])),21)
        self.assertEqual(out['source_candidate_summary.csv'],t['candidate_summary.csv'])
