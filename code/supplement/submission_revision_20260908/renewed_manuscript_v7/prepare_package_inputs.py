...
FINALIZATION_ATTEMPT_PINS=json.loads(r'''{
  "FINALIZATION_ATTEMPT_1.md": "a02fe7b1e2de83526c915ba0783087802c45076af04609a98eab47b3b423661f",
  "FINALIZATION_ATTEMPT_2.md": "974d9362d0d82970bd59798401d434509b1e95c5d02905a8d19d00823d169012",
  "FINALIZATION_ATTEMPT_3.md": "3c26e9db50b3da3dafb87297119ce2383b3d6b23226756ae1c2b8f9e17aae6cd",
  "ROOT_FINAL_QA_CONFIG.json": "021eb630a8f952bc24df58f877e05f07efd19a3b0db86e4840d82055070bae46",
  "ROOT_FINAL_QA_CONFIG_v2.json": "ae694ce5c9d9b3e75216f2dc1a4cb9cb4c66bdf0bec1c39627d9f7bc424f3623"
}''')
...
def main():
    ...
    for source,target in [
        (helper.parent/'READING_SOURCE_SUCCESSOR.md','authoring_provenance/final_qa_tools/READING_SOURCE_SUCCESSOR.md'),
        (HERE/'build_reading_copy.py','authoring_provenance/build_reading_copy.py'),
    ]:
        bound=qa_paths.get(source.resolve())
        if bound is None:raise ValueError('Reading-copy provenance absent from QA: '+str(source))
        add(source,target,'qa_tool_provenance',bound[1])
    for name,digest in FINALIZATION_ATTEMPT_PINS.items():
        add(HERE/'work'/name,'quality/attempts/'+name,'preserved_finalization_attempt',digest)
    for name in ['finalize_v7.py','test_finalize_v7.py','finalize_v7_v2.py','test_finalize_v7_v2.py']:
        source=helper.parent/name
        bound=qa_paths.get(source.resolve())
        if bound is None:raise ValueError('Prior helper absent from QA: '+str(source))
        add(source,'quality/attempts/helpers/'+name,'preserved_finalization_attempt',bound[1])
