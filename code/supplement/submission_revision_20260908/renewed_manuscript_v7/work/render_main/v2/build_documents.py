...
TOC_LABELS={
    ...
    'S9':'S9 Incremental harm calibration',
...
def format_doc(doc,kind,pages):
    ...
    for p in list(doc.paragraphs):
        ...
        if re.match(r'^Table\s+(?:S?\d+)\.',text):
            ...
            if kind=='supplement' and re.match(r'^Table S(?:28|29|30|32|33)\.',text):
    ...
    for table in doc.tables:
        ...
        for i,row in enumerate(table.rows):
            for j,cell in enumerate(row.cells):
                ...
                if kind=='supplement' and len(table.rows)==32 and len(table.columns) in (5,6):
                ...
                for p in cell.paragraphs:
                    keep_whole=(kind=='main' or (kind=='supplement' and table.cell(0,0).text=='Reference' and len(table.columns)==5))
