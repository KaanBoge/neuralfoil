...
DESIGN=HERE.parent/'modern_edition/build_document.py'
spec=importlib.util.spec_from_file_location('preserved_design',DESIGN)
d=importlib.util.module_from_spec(spec);spec.loader.exec_module(d)
TITLE='Measurement informed NeuralFoil drag correction with grouped validation and uncertainty projection'
...
def format_doc(doc,kind,pages):
    ...
    st=d.style(doc,'Title',d.SANS,25,after=16,bold=True)
    ...
    for p in list(doc.paragraphs):
        ...
        if p.style.name=='Title':
            ...
            for run in p.runs:d.font(run,d.SANS,25,'000000',True)
        ...
        if inrefs and re.match(r'^\[\d+\]',text):
            pf.left_indent=Inches(.25);pf.first_line_indent=Inches(-.25);pf.line_spacing=1.12;pf.space_after=Pt(7)
            pf.keep_together=True
            for run in p.runs:d.font(run,d.BODY,10)
        if re.match(r'^Table\s+(?:S?\d+)\.',text):
            ...
            if kind=='supplement' and text.startswith('Table S28.'):
    ...
    for table in doc.tables:
        ...
        for i,row in enumerate(table.rows):
            for j,cell in enumerate(row.cells):
                ...
                if kind=='supplement' and len(table.rows)==32 and len(table.columns)==6:
                    mar=cell._tc.get_or_add_tcPr().find(qn('w:tcMar'))
                    for side in ('top','bottom'):
                        mar.find(qn('w:'+side)).set(qn('w:w'),'40' if table.cell(0,1).text=='Eligible rows' else '50')
                for p in cell.paragraphs:
                    p.paragraph_format.keep_with_next=(i<len(table.rows)-1) if kind=='main' else (i==0)
    ...
    if marker:
        for h in headings:
            ...
            p=marker.insert_paragraph_before();p.paragraph_format.space_after=Pt(4);p.paragraph_format.line_spacing=1.05
            p.paragraph_format.tab_stops.add_tab_stop(Inches(6.7),WD_TAB_ALIGNMENT.RIGHT,WD_TAB_LEADER.DOTS)
            d.hyperlink(p,h['text'],h['anchor'],10.5)
