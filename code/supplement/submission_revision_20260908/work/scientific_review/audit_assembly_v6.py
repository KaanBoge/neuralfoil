...
def main():
    ...
    for method,number in [('fixed_mean8_scale',27),('adaptive_scale',28)]:
        ...
        rows=[l for l in section.splitlines() if l.startswith('| ')][2:]
        assert len(rows)==len(cv[cv.method==method])==31
        for line,(_,r) in zip(rows,cv[cv.method==method].iterrows()):
            ...
            covchecks+=1
    assert covchecks==62
    assert mathstats=={'main':84,'supplement':134},mathstats
    r=dict(status='PASS',main_display_equations_preserved=len(displays),proof_display_equations_preserved=len(proof),native_math_objects_preserved=mathstats,panel_table_rows=checks,coverage_table_rows=covchecks,assembly_sha256=sha(REV/'work/ASSEMBLY.json'),document_sha256={p.name:sha(p) for p in (REV/'deliverables').glob('*.docx')},audit_sha256=sha(Path(__file__)))
    (HERE/'ASSEMBLY_V6_AUDIT.json').write_text(json.dumps(r,indent=2)+'\n');print(r)
