...
def main():
    ...
    for name,old,new in [('main',oa,a),('supplement',ob,b)]:
        ...
        changes[name]=delta
    assert [c['tag'] for c in changes['main']]==['replace','replace','replace']
    ...
    c=changes['main'][2]
    assert c['old_text'].strip()==oldrep.strip() and c['new_text'].strip()==newrep.strip()
    ...
    result={'status':'PASS_CURRENT_V8_EDITORIAL_ASSEMBLY_REAUTHENTICATION','created_UTC':datetime.datetime.now(datetime.timezone.utc).isoformat(),
      ...
      'all_tables_byte_unchanged':True,'all_references_unchanged':True,'main_changed_blocks':3,'supplement_changed_blocks':1,
      ...
      'changed_blocks':changes,'historical_review_is_not_rebound_to_current_files':True,'assembly_input_count':len(m['source_sha256'])}
