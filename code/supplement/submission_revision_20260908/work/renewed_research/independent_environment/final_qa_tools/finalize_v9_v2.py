...
def pointer(d,path):
    ...
    return d
class Pins:
    def __init__(self,root,external_readonly_pins=None):
        self.root=Path(root).resolve();self.files={};self.external={}
        require(external_readonly_pins is None or type(external_readonly_pins) is dict,'Exact external file map')
        for raw,h in (external_readonly_pins or {}).items():
            p=self.external_path(raw)
            require(not p.is_relative_to(self.root),'External list must not replace project rules')
            require(type(h) is str and re.fullmatch('[0-9a-f]{64}',h) is not None,'External digest')
            require(str(p) not in self.external,'Duplicate external alias')
            self.external[str(p)]=h
    def external_path(self,raw):
        require(type(raw) is str and raw.startswith('/') and raw==str(Path(raw))
                and not any(x in {'.','..',''} for x in raw.split('/')[1:])
                and not any(ord(c)<32 or ord(c)==127 for c in raw),'Canonical absolute external file only')
        p=Path(raw)
        require(not any(x.is_symlink() for x in [p,*p.parents]),'External symlink forbidden')
        require(p.is_file(),'External regular file required')
        return p
    def path(self,s):
        p=Path(s);p=p if p.is_absolute() else self.root/p
        require(p.is_relative_to(self.root),'External path cannot be an output or project path')
        ...
        return p
    def pin(self,spec):
        raw=spec['path'];h=spec['sha256']
        if type(raw) is str and Path(raw).is_absolute() and not Path(raw).is_relative_to(self.root):
            require(raw in self.external and self.external[raw]==h,'Unlisted external file or digest')
            p=self.external_path(raw)
        else:p=self.path(raw)
    ...
    def reread(self):
        for p,h in self.files.items():
            if p in self.external:self.external_path(p)
            require(sha(Path(p))==h,'Concurrent mutation: '+p)
...
def run(config_path,output,copy_pdfs=False):
    ...
    pins=Pins(c['project_root'],c.get('external_readonly_pins'));output=pins.path(output)
    ...
    result={'status':'PASS_TECHNICAL_PREPARATION','scope':'Internal computational QA only; no author approval, rights clearance, submission, or physical accuracy certification',
      ...
      'external_readonly_files':{p:h for p,h in pins.files.items() if p in pins.external},
