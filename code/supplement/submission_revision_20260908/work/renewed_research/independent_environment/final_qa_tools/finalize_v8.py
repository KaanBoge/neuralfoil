...
COUNTS={'main':(17,133,4,4),'supplement':(39,298,35,2)}
REFERENCES=21
...
def check_counts(kind,actual):
    require(kind in COUNTS and tuple(actual)==COUNTS[kind] and all(type(n) is int for n in actual),'Content counts mismatch')
def check_references(numbers):
    require(numbers==list(range(1,REFERENCES+1)) and all(type(n) is int for n in numbers),'Reference sequence')
def validate_config(c):
    require(c['schema']=='v8-final-qa-1','Config schema')
    require(c.get('finalization_authorized') is True,'Draft config is not authorized for finalization')
    expected={**{k:list(v) for k,v in COUNTS.items()},'references':REFERENCES}
    require(c.get('content_contract')==expected,'Explicit V8 content contract required')
    require(all(type(n) is int for k in COUNTS for n in c['content_contract'][k]) and type(c['content_contract']['references']) is int,'Integer V8 content contract')
...
def document(pins,kind,cfg):
    ...
    check_references([int(m[1]) for p in doc.paragraphs if (m:=re.match(r'^\[(\d+)\]',p.text))])
    if kind=='supplement':
