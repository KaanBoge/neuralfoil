...
AUDIT_SHA = 'e2f0db2b508826556a7a66aa19a2e49f59075d4424386fe143d04714cea355aa'
AUDIT_PATH = 'submission_revision_20260908/work/renewed_research/uncertainty_review/range_bound_feasibility/four_downstream_audit/V9_ASSEMBLY_V2_QA.json'
AUDIT_ASSERTIONS = [{'pointer': '/result', 'equals': 'PASS'}]
...
def validate_binding(b):
    ...
    audit = b['assembly_audit']
    require(audit.get('role') == 'v9_assembly_audit' and audit.get('assertions') == AUDIT_ASSERTIONS,
            'Exact completed audit /result PASS assertion required')
    selected_path = Path(audit['path'])
    if not selected_path.is_absolute(): selected_path = HERE.parents[4]/selected_path
    require(selected_path == HERE.parents[4]/AUDIT_PATH and audit['sha256'] == AUDIT_SHA,
            'Exact selected V9 assembly v2 audit required')
...
def prepare(root, binding_path, binding_sha):
    ...
    require(audit.get('result') == 'PASS', 'Selected audit is not PASS')
