...
import finalize_v9_v2 as q
import io,zipfile
from pypdf.generic import DictionaryObject,NameObject,ArrayObject

class ExternalReadOnly(unittest.TestCase):
    def setUp(self):
        self.temp=tempfile.TemporaryDirectory();self.base=Path(self.temp.name).resolve()
        self.root=self.base/'project';self.root.mkdir()
        self.ext=self.base/'external.py';self.ext.write_text('source bytes')
        self.mapping={str(self.ext):q.sha(self.ext)}
    def tearDown(self):self.temp.cleanup()
    def test_exact_read_hash_and_reauthentication(self):
        pins=q.Pins(self.root,self.mapping)
        self.assertEqual(pins.pin({'path':str(self.ext),'sha256':q.sha(self.ext)}),self.ext)
        pins.reread();self.ext.write_text('mutated')
        with self.assertRaises(ValueError):pins.reread()
    def test_unlisted_and_digest_mismatch(self):
        for mapping,h in [({},q.sha(self.ext)),(self.mapping,'0'*64)]:
            with self.assertRaises(ValueError):q.Pins(self.root,mapping).pin({'path':str(self.ext),'sha256':h})
    def test_external_outputs_never_allowed(self):
        pins=q.Pins(self.root,self.mapping)
        with self.assertRaises(ValueError):pins.path(str(self.ext))
        with self.assertRaises(ValueError):pins.path(self.base/'new-output.json')
    def test_prefix_directory_relative_and_traversal_rejected(self):
        for raw in [str(self.base),'external.py',str(self.base)+'/../'+self.base.name+'/external.py',str(self.base)+'/./external.py',str(self.base)+'//external.py',str(self.ext)+'/']:
            with self.assertRaises(ValueError):q.Pins(self.root,{raw:'a'*64})
    def test_symlink_file_and_ancestor_rejected(self):
        link=self.base/'link.py';link.symlink_to(self.ext)
        with self.assertRaises(ValueError):q.Pins(self.root,{str(link):q.sha(self.ext)})
        alias=self.base/'alias';alias.symlink_to(self.base,target_is_directory=True)
        with self.assertRaises(ValueError):q.Pins(self.root,{str(alias/'external.py'):q.sha(self.ext)})
    def test_symlink_replacement_rejected(self):
        pins=q.Pins(self.root,self.mapping);pins.pin({'path':str(self.ext),'sha256':q.sha(self.ext)})
        identical=self.base/'identical.py';identical.write_bytes(self.ext.read_bytes())
        self.ext.unlink();self.ext.symlink_to(identical)
        with self.assertRaises(ValueError):pins.reread()
    def test_external_alias_into_project_rejected(self):
        internal=self.root/'source';internal.write_text('data')
        alias=self.base/'project-link';alias.symlink_to(self.root,target_is_directory=True)
        with self.assertRaises(ValueError):q.Pins(self.root).path(alias/'source')
        with self.assertRaises(ValueError):q.Pins(self.root).pin({'path':str(alias/'source'),'sha256':q.sha(internal)})
    def test_project_entry_not_external_override(self):
        internal=self.root/'source';internal.write_text('data')
        with self.assertRaises(ValueError):q.Pins(self.root,{str(internal):q.sha(internal)})
    def test_bad_map_and_digest(self):
        for mapping in [[],[(str(self.ext),'a'*64)],{str(self.ext):None},{str(self.ext):'xyz'}]:
            with self.assertRaises(ValueError):q.Pins(self.root,mapping)
