...
DOUBLE-CLEAN RERUN with the full sixteen-token comment filter (export audit 2026-09-06); writes dc- outputs only.
...
Writes dc-lift-report.txt and dc-clmax.csv.
"""
    ...
    while i < n:
        ...
        if L.startswith("Airfoil:"):
            ...
            if key in GEO and GEO[key]["config"] == "clean" and not any(
                    t in comment.lower() for t in ("trip", "tape", "zigzag", "zig-zag", "rough", "bump", "flap", "gurney", "turbulator", "vortex", "serrat", "upside", "inverted", "gap", "seal", "cover")):
                for k in range(cnt):
                    i += 1
                    p = txt[i].split()
                    try:
...
def _geom_text(path):
    ...
    if os.path.exists(full): return open(full, errors="replace").read()
    if path.startswith("coords/"): return _ZC.read(path[7:]).decode("latin-1")
    if path.startswith("stec8/"): return _ZS.read(path[6:]).decode("latin-1")
    raise FileNotFoundError(path)
def load_pts(path):
    pts = []
    for L in _geom_text(path).splitlines()[1:]:
        p = L.split()
        if len(p) >= 2:
            try:
...
rep = open(os.path.join(BASE, "dc-lift-report.txt"), "w")
def P(*a):
