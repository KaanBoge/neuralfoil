...
DECLARED BEFORE FITTING, no tuning afterward:
  model: sklearn GradientBoostingRegressor(n_estimators=300, learning_rate=0.06,
         max_depth=3, min_samples_leaf=80, subsample=0.7, random_state=824)
...
Exports correction-cd3.json (+cl) with raw trees, a numeric export check, and
...
FEATS = ["alpha", "lre", "t2", "c2", "lcd8", "cl8", "lsp", "topxtr", "botxtr"]
...
for r in csv.DictReader(open(os.path.join(BASE, "lsat-nf2.csv"))):
    ...
    X.append([a, np.log10(Re), s2["t"], s2["c"],
              np.log(cd8), cl8, np.log(spread),
...
def gbr():
    return GradientBoostingRegressor(n_estimators=150, learning_rate=0.06, max_depth=2,
...
okCL, oofCL = False, np.zeros(len(yCL))  # CL already shipped from v2; this attempt is CD-only by declaration
...
if okCD:
    export(gbr().fit(X, yCD), "correction-cd3.json", {"kind": "logratio", "clip": 1.5})
if False:
    export(gbr().fit(X, yCL), "correction-cl3-unused.json", {"kind": "additive", "clip": 0.5})
with open(os.path.join(BASE, "oof3.csv"), "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["entry", "Re", "alpha", "zCD_oof", "dCL_oof"])
    for i, mm in enumerate(meta):
        w.writerow([mm[0], mm[1], mm[2], f"{oofCD[i]:.5f}", f"{oofCL[i]:.5f}"])
print("wrote oof3.csv")
