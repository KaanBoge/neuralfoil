...
Output lsat-nf.csv: one row per measured point with per-size CL/CD + xlarge
...
out = open(os.path.join(BASE, "lsat-nf.csv"), "w", newline="")
w = csv.writer(out)
w.writerow(["source", "airfoil", "config", "geom_kind", "Re", "alpha", "CL_meas", "CD_meas", "u_cd_span"]
           + [f"CL_{s}" for s in SIZES] + [f"CD_{s}" for s in SIZES] + ["conf_xlarge", "tc"])
