...
## PART 12. The accuracy answer: a validated correction, and the head-to-head with XFOIL (2026-08-29, night)

**The first repair to survive validation.** A measured-residual drag correction was fit on the LSAT corpus with the protocol declared before fitting: a fixed 21-term basis in (log Re, alpha, t/c, log predicted CD, log ensemble spread, confidence, predicted CL), ridge penalty fixed in advance, target the clipped log ratio of measured to predicted drag, and a ship rule requiring improvement on airfoil-disjoint 5-fold cross-validation AND both cross-facility transfer directions. It passed all three: 37.9 to 33.9 counts on airfoils it never saw (+10.5 percent), vols-to-SoarTech 32.1 to 29.9, SoarTech-to-vols 41.8 to 38.6. It ships in the new NeuralFoil, fading smoothly to zero outside its measured domain (Re above 600,000, |alpha| above 12 degrees, t/c outside 5 to 20 percent, Mach above 0.3), so nothing outside that domain is touched. Every earlier repair attempt in this research failed held-out testing with 139 transonic points; this one had 10,312 in-domain points and 127 airfoils to learn from, which is the whole difference.

**The head-to-head against the standard tool.** XFOIL 6.99 (n_crit 9, free transition, 200 iterations, the field's de facto standard and NeuralFoil's own teacher) was run on every clean corpus block. It converged on 74.7 percent of the measured points; it gives no answer at all on the remaining quarter. On the 7,897 common converged points, mean absolute drag error against measurement: XFOIL 40.8 counts, classic NeuralFoil xlarge 35.8, new NeuralFoil mean-of-8 35.5, new NeuralFoil with the out-of-fold measured correction 33.3. Lift: XFOIL 0.089, classic 0.085, new 0.082. The corrected new NeuralFoil is the most accurate of the four in every Reynolds band (54 versus XFOIL's 68 counts below Re 75,000; 24 versus 28 at Re 250,000 to 600,000).

**The honest scope of the claim.** This regime is Re 39,000 to 504,000 at low Mach on smooth clean models, XFOIL scored only where it converged (its best case), and the surrogate beating its teacher against reality is explained by ensemble averaging over eight networks, a correction learned from measurement, and the teacher's own convergence failures. High-Reynolds and transonic accuracy remain governed by Parts 1 to 9: the drag-rise magnitude is still a no-trust zone, and no measured data exists between Re 600,000 and the transonic reports.

Files: correction-cd.json (coefficients and validation record), lsat_correction.py, lsat_xfoil.py, lsat_compare.py, lsat-xfoil.csv, lsat-headtohead.txt.

## PART 13. Corrections v2 and v3: one rejection, two ships, and the final accuracy standing (2026-08-30)

**The attempt history, in full.** After the v1 ridge correction (Part 12), a gradient-boosted model with 16 features including transition locations and geometry descriptors was declared and run. For drag it scored +15.7 percent on airfoil-disjoint folds but FAILED one cross-facility transfer (41.8 to 42.1 counts on the volumes when trained on SoarTech): the trees had learned facility-specific residual structure, and the declared ship rule rejected it. For lift the same model class PASSED everything: +26.7 percent on unseen airfoils, improving in both transfer directions. One further drag attempt was then declared with an invariance-focused design (nine physics-only features, halved depth and tree count, doubled leaf size) and PASSED: +12.4 percent on unseen airfoils, both transfers improving. Shipped: drag v3 and lift v2, as exported tree ensembles verified machine-exact against sklearn and self-tested in the page on load. The v2 drag rejection is part of the record.

**The final head-to-head (out-of-fold corrections, 7,897 common points where XFOIL converged):** drag MAE: XFOIL 40.8 counts, classic NeuralFoil 35.8, new NeuralFoil 32.5 (best in every Reynolds band: 52 versus XFOIL's 68 below Re 75,000; 23 versus 28 at Re 250,000 to 600,000). Lift MAE: XFOIL 0.089, classic 0.085, new 0.058. Against the standard tool that is 20 percent better drag and 35 percent better lift; against the classic NeuralFoil, 9 percent better drag and 32 percent better lift, on measurement neither correction ever trained on.

**Scope unchanged:** Re 39,000 to 504,000, low Mach, clean smooth models; corrections fade to zero outside their measured domain; the transonic drag-rise magnitude remains a displayed no-trust zone; XFOIL is scored only where it converged (74.7 percent of points).

Files: correction-cd3.json, correction-cl2.json (shipped, with reference vectors), lsat_correction2.py, lsat_correction3.py, lsat_run2.py, lsat_geofeat.py, lsat_compare2.py, oof2.csv, oof3.csv, lsat-headtohead2.txt.

## PART 14. The export audit: a contaminated filter, a withdrawn correction, and the corrected standing (2026-09-06)

**How it was found.** Exporting the complete working folder for the paper triggered an independent read-only audit of the export's own documents against every file. Two of its nineteen findings concerned the published science rather than the export.

**Finding one: the corpus filter was inconsistent.** The parser recorded each measured row's configuration from the report's Comment field, but the geometry matcher classified entries from the airfoil name alone, and every downstream script used the name-based label. Volumes 1 to 3 list trip and other modified runs under the plain airfoil name with the modification stated only in the Comment, so 1,974 of the 10,608 rows published as "clean" (18.6 percent; 1,934 distinct conditions; 27 entries, led by SD7037 (E) with 217 rows, S7075 (A) 200, SG6042 178 and RG15 (C) 176) were measurements of tripped or otherwise modified configurations compared against free-transition predictions. The double-clean corpus, clean by both name and comment, is 8,634 rows over 135 airfoils.

**Finding two: the published XFOIL convergence figure was wrong.** XFOIL had in fact been submitted only the double-clean conditions, 8,705 of them, and converged on 7,897 (90.7 percent; 91.3 percent of the loadable conditions once DF103's placeholder geometry is set aside). The published 74.7 percent used a denominator that counted 1,862 conditions never sent to XFOIL. The head-to-head error figures were unaffected because they were computed only on common converged points, but the statement that XFOIL fails on a quarter of conditions was false; it fails on about nine percent.

**The re-analysis.** The entire low-Reynolds analysis was repeated on the double-clean rows with the identical declared protocols (lsat_doubleclean.py; geometry read from the original archive streams because the extracted copies had been deleted by the operating system after the runs, see the export's missing-files report). The core result holds: the mean of eight beats the classic xlarge on 56 percent of points (drag MAE 37.4 against 38.0 counts, lift 0.0816 against 0.0843). Removing the tripped runs lowers every error median (Re 150,000 to 250,000: 10.9 counts; 250,000 to 350,000: 8.1; 350,000 to 600,000: 7.3, all at or below the measurement's own spanwise half-spread of 10.5), the spread deciles still rank true error monotonically (7.3 to 96.9 counts), and the confidence blindspot is 35.4 percent instead of 38.9.

**The drag correction is withdrawn.** Retrained on the double-clean corpus under the same declared rule, the drag correction v3 improves airfoil-disjoint cross-validation by 15.4 percent but worsens the SoarTech-to-volumes transfer (34.8 to 35.9 counts). By the rule it does not ship. Its earlier pass was therefore an artifact of the contaminated validation set: the model had partly learned the drag offset of tripped runs that appeared in both training and test folds. It was removed from the site on 2026-09-06 and the shipped drag prediction is the uncorrected mean of eight.

**The lift correction survives and strengthens.** On the double-clean corpus the lift correction v2 improves airfoil-disjoint cross-validation by 35.4 percent (0.0811 to 0.0524) and both transfers (SoarTech 0.090 to 0.059; volumes 0.072 to 0.062). It ships retrained (dc-correction-cl2.json), verified machine-exact against the Python model.

**The corrected head-to-head (7,880 XFOIL-converged double-clean points, corrections out of fold).** Drag MAE: XFOIL 40.8 counts, classic NeuralFoil 35.8, new NeuralFoil 35.5 (13 percent better than XFOIL, one percent better than classic). Lift MAE: XFOIL 0.089, classic 0.085, new NeuralFoil with the lift correction 0.054 (40 percent better than XFOIL, 37 percent better than classic). The earlier claims of 20 percent and 9 percent on drag are withdrawn together with the drag correction.

**What the lift-file corpus needed.** The separate lift-curve corpus (Part 11) already filtered its Comment field, but with a five-token list rather than the full sixteen-token list the drag parser used; a rerun with the full list (lsat_lift_dc.py) is reported in the addendum that follows this part.

**What this episode says about the method.** The pre-declared ship rule caught the drag correction the moment it was tested on honest data, exactly as it was designed to. What the rule cannot catch is a contaminated validation set, which is why the audit that found it, an independent re-measurement of every published number against the files, is now part of the record, and why the export that triggered it is the paper's supplementary material.

Files: lsat_doubleclean.py, dc-report.txt, dc-oof.csv, dc-correction-cl2.json, dc-by-airfoil.csv, and the export records export-manifest-2026-09-06.md, export-missing-files-2026-09-06.md, export-unused-data-2026-09-06.md.

**Part 14 addendum: the lift-file corpus under the full filter (2026-09-06, later the same day).** The part 11 lift-curve corpus had excluded comment fields containing five modification tokens; rerun with the same sixteen-token list the drag parser uses (lsat_lift_dc.py, reading the deleted LIFT02 and LIFT03 members and the SoarTech lift files from the original archives in memory), the corpus shrinks from 31,075 points on 108 airfoils to 17,470 points on 76 airfoils and 354 sweeps, because the wider list also removes runs whose comments mention sealed gaps, coverings and similar details. Every part 11 conclusion survives on the stricter set, and the numbers move slightly in the model's favor: pre-stall CL MAE 0.0819 for the mean of eight against 0.0860 for the classic (part 11: 0.0902 against 0.0937), near-stall 0.110 against 0.114, negative alpha 0.067 against 0.071; on the 351 sweeps with a captured lift peak, CLmax is overpredicted on 74 percent of sweeps with mean bias +0.049 and MAE 0.073 (classic 0.081), and the stall angle is called 0.92 degrees early on average (MAE 2.19 degrees). Both sets are published (lsat-lift-report.txt, dc-lift-report.txt, dc-clmax.csv); the stricter one governs.
